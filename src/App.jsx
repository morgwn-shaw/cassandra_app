...
const App = () => {
    ...
    const [status, setStatus] = useState({ active_engine: 'Awaiting Ping...' });
    ...
    const testConnection = async () => {
        ...
        try {
            ...
            window.alert(`HANDSHAKE SUCCESS: ${data.status}\nEngine: ${data.engine}`);
            setStatus({ active_engine: data.engine });
        } catch (e) {
            window.alert(`HANDSHAKE FAILED: ${e.message}`);
    ...
    const runAction = async (endpoint, body) => {
        ...
        try {
            ...
            const data = await res.json();
            if (data.error) window.alert(`BACKEND: ${data.error}`);
            await refreshData();
        } catch (e) { window.alert(`FAULT: ${e.message}\nURL: ${url}`); }
        finally { setLoading(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0a0c0e] text-slate-400 select-none">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Zap size={16} /> Telemetry_v47.5</div>
                    <div className="text-[10px] text-teal-900 uppercase font-black italic mt-2">{status?.active_engine}</div>
                </div>
                <button onClick={testConnection} className="w-full p-4 border border-teal-900/30 bg-teal-500/5 text-teal-500 text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-teal-500 hover:text-black transition-all">
                    <Wifi size={14}/> Test_Handshake
                </button>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {loading && (
                    <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase tracking-[1em] animate-pulse italic">Hard-Linking_Apex...</p>
                    </div>
                )}
                ...
                {!activeItem ? (
                    ...
                    <div className="animate-in slide-in-from-bottom-8 duration-500 space-y-12">
                                ...
                                {viewMode === 'god' && (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><History size={18}/> Shared_Lore</h4>
                                        ...
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><FastForward size={18}/> Roadmap</h4>
                                            <p className="text-[15px] text-slate-400 font-sans p-4">{activeItem?.lore?.future_lore}</p>
                                        </div>
                                    </div>
                                )}
                        ...
                        {/* PERSONA DETAIL VIEW IDENTICAL TO v43.5 */}
                        {activeTab === 'persona' && (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 animate-in slide-in-from-bottom-10">
                                <div className="bg-black/40 p-12 border border-slate-800 rounded-3xl h-[750px] overflow-y-auto custom-scrollbar">
                                    <h4 className="text-teal-500 text-[12px] font-black uppercase mb-8 italic border-b border-slate-800 pb-4 tracking-widest"><Quote size={20}/> Forensic Dossier</h4>
