...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, ClipboardCheck, Terminal, AlertCircle } from 'lucide-react';
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "SYSTEM_READY", type: "system" }]);
    ...
    const sync = useCallback(async () => {
        try {
            ...
            if (resP.ok && resS.ok) {
                ...
                addLog("LIBRARY_SYNC_OK", "system");
            }
        } catch (e) { addLog(`SYNC_ERROR: ${e.message}`, "error"); }
    }, []);

    useEffect(() => { sync(); }, [sync]);

    const createPersona = async () => {
        setLoad(true); setStatus("Spawning...");
        addLog(`INIT_DNA_SPAWN: ${nP.name}`);
        try {
    ...
    const createSeason = async () => {
        setLoad(true); setStatus("Reconciling...");
        addLog(`INIT_SEASON: ${nS.topic}`);
        try {
    ...
    const runProduction = async (epIdx) => {
        setLoad(true);
        let finalScript = "";
        addLog(`START_PRODUCTION: EP_${epIdx + 1}`);
        try {
            for (let i = 1; i <= 6; i++) {
                setStatus(`ACT ${i} PRODUCING...`);
                addLog(`REQUESTING_ACT_${i}...`, "system");
                ...
                addLog(`ACT_${i}_RECEIVED: ${data.script?.substring(0, 50)}...`, "llm");
                
                if (!data.script) {
                    addLog(`ACT_${i}_MISSING_CONTENT`, "error");
                    throw new Error(`Act ${i} came back empty.`);
                }
            ...
            addLog("EPISODE_PRODUCTION_BIBLE_SAVED", "system");
            await sync();
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            {/* MONITORING PANE (NEW) */}
            <aside className="w-[300px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0 overflow-hidden">
                <div className="p-4 border-b border-slate-800 flex items-center gap-2 text-teal-500 font-black uppercase tracking-widest italic">
                ...
                <div ref={logRef} className="flex-1 overflow-y-auto p-4 space-y-3 custom-scrollbar text-[9px] uppercase leading-tight bg-black/20">
                    {logs.map((log, i) => (
                        <div key={i} className={`p-2 rounded border ${log.type === 'error' ? 'bg-red-950/20 border-red-900/40 text-red-500' : log.type === 'llm' ? 'bg-teal-950/20 border-teal-900/40 text-teal-400' : 'bg-slate-900/40 border-slate-800 text-slate-500'}`}>
                            <div className="flex justify-between font-black mb-1 opacity-50">
                                <span>[{log.type}]</span>
                                <span>{log.t}</span>
                            </div>
                            <div className="break-words select-text">{log.m}</div>
                        </div>
                    ))}
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); setActiveEp(null); }} className={`px-8 py-3 font-black uppercase transition-all rounded-lg ${!activeId ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800 hover:bg-slate-700'}`}>Library</button>
                    {activeId && (
                        <>
                    ...
                    <div className="grid grid-cols-2 gap-8 overflow-y-auto custom-scrollbar">
                        {seasons.map(s => (
                            <div key={s.id} onClick={() => setActiveId(s.id)} className="bg-[#1c1f23] p-10 border border-slate-800 rounded-[2rem] cursor-pointer hover:border-teal-500 transition-all relative group shadow-2xl">
                                ...
                                <div className="mt-3 text-teal-600 font-black uppercase italic">{s.rel} | Archetype: {s.archetype}</div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="flex flex-col h-full gap-8 overflow-hidden">
                        {activeEp !== null ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-12 flex flex-col gap-8 overflow-hidden relative shadow-inner">
                                <button onClick={() => setActiveEp(null)} className="absolute top-8 right-12 text-slate-500 hover:text-white font-black uppercase italic tracking-widest">[ Exit ]</button>
                                    ...
                                    {activeSeason.episodes[activeEp].full_script ? (
                                        <>
                                            ...
                                            <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl">
                                                ...
                                                <p className="text-slate-400 uppercase leading-relaxed select-text">{activeSeason.episodes[activeEp].assessment}</p>
                                            </div>
                                        </>
                                    ) : (
            ...
            <aside className="w-[340px] bg-black/60 p-8 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <select className="w-full bg-slate-900/50 p-5 border border-slate-800 text-teal-400 rounded-2xl outline-none font-black" value={nS.relationship} onChange={e => setNS({...nS, relationship: e.target.value})}>{CONFIG.D.map(d => <option key={d} value={d}>{d}</option>)}</select>
