...
const App = () => {
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            if (!r.ok) throw new Error(d.error || "Signal Interrupted");
            setEng(d.model?.toUpperCase()); setToday(d.date);
            window.alert(`SIGNAL SUCCESS: ${d.model}`);
        } catch (e) { window.alert(`SIGNAL FAIL: ${e.message}`); } finally { setLoad(false); }
    };
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Establishing Signal Lock...</p></div>}
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="space-y-3">
                                    {(active?.episodes || []).map((e, idx) => (
                                        <div key={idx} className="bg-[#1c1f23] p-5 border border-slate-800 rounded-2xl flex justify-between items-center group shadow-lg">
