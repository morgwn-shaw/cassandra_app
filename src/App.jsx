...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "CORE_READY_V186", type: "system" }]);
    ...
    const [nP, setNP] = useState({ name: '', role: 'Host', gender: CONFIG.G[0], description: "A witty historian with a love for obscure artifacts." });
    ...
    const createSeason = async () => {
        if (!nS.topic || nS.host_ids.length < 2) {
            addLog("PRE_FLIGHT_FAIL: Missing Topic or Hosts", "error");
            return;
        }
        setLoad(true);
        addLog(`ESTABLISHING: ${nS.topic}`, "system");
        try {
            setStatus("Step 1: Initializing...");
            const r1 = await fetch(`${BASE_URL}/season/init`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(nS) });
            const s = await r1.json();
            addLog(`SKELETON_OK: ID ${s.id}`, "system");

            setStatus("Step 2: Researching...");
            addLog("AI_RESEARCH_START...");
            ...
            addLog("EPISODES_MAPPED", "system");

            setStatus("Step 3: History Sync...");
            addLog("SHARED_LORE_START...");
            ...
            await sync();
        } catch (e) { addLog(`FATAL: ${e.message}`, "error"); }
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0 overflow-hidden">
                ...
                <div ref={logRef} className="flex-1 overflow-y-auto p-4 space-y-2 bg-black/20 custom-scrollbar text-[9px] uppercase">
                    {logs.map((log, i) => <div key={i} className={log.type === 'error' ? 'text-red-500' : log.type === 'system' ? 'text-teal-500' : ''}>[{log.t}] {log.m.substring(0, 60)}</div>)}
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); setActiveEp(null); }} className={`px-8 py-3 font-black uppercase rounded-lg ${!activeId ? 'bg-teal-500 text-black' : 'bg-slate-800 hover:bg-slate-700'}`}>Library</button>
            ...
            <aside className="w-[340px] bg-black/60 p-8 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                <div className="space-y-6">
                    ...
                    <select className="w-full bg-slate-900/50 p-4 border border-slate-800 text-teal-500 rounded-xl outline-none uppercase font-black" value={nP.gender} onChange={e => setNP({...nP, gender: e.target.value})}>{CONFIG.G.map(g => <option key={g} value={g}>{g}</option>)}</select>
                    ...
                    <div className="flex flex-wrap gap-4 pt-6 border-t border-slate-900">
                        {personas?.map(p => <img key={p.id} src={p.portrait} onClick={() => setViewPersona(p)} className="w-14 h-14 rounded-xl border-2 border-slate-800 hover:border-teal-500 cursor-pointer bg-black" alt="p" />)}
                    </div>
                </div>
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <input className="w-full bg-slate-900/50 p-5 border border-slate-800 text-white font-bold rounded-xl outline-none focus:border-teal-500 uppercase text-[12px]" placeholder="TOPIC" value={nS.topic} onChange={e => setNS({...nS, topic: e.target.value})} />
