...
const DYNAMICS = ["Unresolved Sexual Tension (UST)", "UST - Denied Attraction", "Grudging Mutual Respect", "Mentor / Protégé (Estranged)", "Bitter Professional Rivals", "Two Strangers sharing a Secret", "Cold War (Silent Treatment)"];
const TRAUMA_POOL = ["Witnessed a server farm bleed-out.", "Accidentally decrypted a child's last words.", "Betrayed by a neural-link mentor.", "Identity stolen for high-profile hits.", "Escaped a digital singularity cult."];
...
const App = () => {
  ...
  const [newP, setNewP] = useState({ name: '', role: 'Victim Advocate / Tech Cynic', trauma: '', gender: 'Random' });
  ...
  const handleCreatePersona = async () => {
    ...
    refreshData();
    setNewP({ name: '', role: 'Victim Advocate / Tech Cynic', trauma: '', gender: 'Random' });
    setLoading(false);
  };
    ...
    <div className="h-screen w-screen bg-[#0d0f11] text-slate-300 font-mono flex overflow-hidden">
      ...
      <section className="w-[420px] border-r border-slate-800 bg-[#08090a] flex flex-col p-6 shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between mb-8 border-b border-slate-800 pb-4">
            <div className="text-teal-400 font-black uppercase text-[10px] tracking-[0.4em] flex items-center gap-2">
                <Zap size={14} className={loading ? "animate-pulse text-amber-500" : ""} /> Kernel_v26.4
            </div>
      ...
      <section className="flex-1 flex flex-col bg-[#0d0f11] p-10 overflow-y-auto border-r border-slate-800 relative">
        <div className="flex gap-4 mb-10 border-b border-slate-800 pb-6">
            <button onClick={() => { setActiveTab('season'); setActiveItem(null); setActiveBrief(null); }} className={`px-10 py-3 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'season' && !activeItem ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800 text-slate-500 hover:text-white'}`}>Seasons</button>
            <button onClick={() => { setActiveTab('persona'); setActiveItem(null); setActiveBrief(null); }} className={`px-10 py-3 text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'persona' && !activeItem ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800 text-slate-500 hover:text-white'}`}>DNA_Vault</button>
        </div>
        ...
        {/* DETAIL VIEW: NODES + SHOWRUNNER BRIEF */}
            ...
            <div className="animate-in fade-in space-y-12">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="space-y-6 bg-black/20 p-8 border border-slate-800 rounded-xl">
                        <h4 className="text-teal-500 text-[10px] font-black uppercase mb-4 italic border-b border-slate-800 pb-2">Macro_Nodes</h4>
                    ...
                    <div className="space-y-6 bg-black/20 p-8 border border-slate-800 rounded-xl">
                        <h4 className="text-teal-500 text-[10px] font-black uppercase mb-4 italic border-b border-slate-800 pb-2">6-Act_Output</h4>
                        {activeBrief ? (
        ...
        <div className="space-y-6 bg-black/30 p-8 rounded-2xl border border-slate-800 shadow-xl">
            ...
            <div className="space-y-4">
                ...
                <div className="relative">
                   <input className="w-full bg-[#161a1d] p-4 border border-slate-800 text-[11px] text-slate-500 outline-none uppercase pr-12" placeholder="CORE_TRAUMA" value={newP.trauma} onChange={(e) => setNewP({...newP, trauma: e.target.value})} />
                   <button onClick={() => setNewP({...newP, trauma: TRAUMA_POOL[Math.floor(Math.random() * TRAUMA_POOL.length)]})} className="absolute right-4 top-3.5 text-slate-700 hover:text-teal-500 transition-all"><Dice5 size={20}/></button>
                </div>
                <button onClick={handleCreatePersona} disabled={loading || !newP.name} className={`w-full py-4 text-[10px] font-black uppercase tracking-widest rounded shadow-xl transition-all ${loading || !newP.name ? 'bg-slate-900 text-slate-800 opacity-20 cursor-not-allowed' : 'bg-teal-500 text-black hover:bg-white'}`}>Commit_DNA</button>
            </div>
        </div>
