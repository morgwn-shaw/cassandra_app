        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0d0f11] text-slate-400">
            ...
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-6 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-2 text-teal-500 font-black text-[10px] uppercase tracking-widest"><Zap size={14} /> Telemetry_v36.0</div>
                    <div className="text-[8px] text-teal-900 uppercase font-black italic">{status.active_engine}</div>
                </div>
                ...
                <button onClick={() => runAction('/purge', {})} className="p-3 bg-red-950/20 border border-red-900/30 text-red-500 text-[9px] font-black uppercase flex items-center justify-center gap-2 hover:bg-red-600 transition-all">
            ...
            <section className="flex-1 flex flex-col p-10 overflow-y-auto relative bg-[#121416]">
                {loading && (
                    <div className="absolute inset-0 bg-black/80 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-[10px] font-black uppercase animate-pulse">Syncing_Apex_Signals...</p>
                    </div>
                )}

                <div className="flex gap-4 mb-10 border-b border-slate-800 pb-6">
                    <button onClick={() => {setActiveItem(null); setActiveBrief(null); setActiveTab('season');}} className={`px-10 py-3 text-[10px] font-black transition-all ${activeTab === 'season' && !activeItem ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800 text-slate-500'}`}>SEASONS</button>
                    <button onClick={() => {setActiveItem(null); setActiveBrief(null); setActiveTab('persona');}} className={`px-10 py-3 text-[10px] font-black transition-all ${activeTab === 'persona' && !activeItem ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800 text-slate-500'}`}>DNA_VAULT</button>
                </div>

                {!activeItem ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in fade-in">
                        {currentList.map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-8 rounded cursor-pointer hover:border-teal-500/50 transition-all flex gap-6 items-center shadow-xl group" onClick={() => setActiveItem(item)}>
                    ...
                    <div className="animate-in slide-in-from-bottom-4 space-y-12">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-8">
                            ...
                            <button onClick={() => {setActiveItem(null); setActiveBrief(null);}} className="bg-slate-800 text-white px-8 py-3 text-[10px] font-black hover:bg-teal-500 transition-all">CLOSE</button>
                        </div>

                        {activeTab === 'season' && (
                            <div className="flex flex-col gap-12">
                                <div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-xl shadow-2xl">
                                    <h4 className="text-teal-400 text-[10px] font-black uppercase mb-6 italic tracking-[0.2em] flex items-center gap-3"><ScrollText size={18}/> Arc_Summary</h4>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                                    <div className="space-y-4">
                                        <h4 className="text-teal-500 text-[10px] font-black uppercase italic border-b border-slate-800 pb-2">Investigative_Nodes</h4>
                                        {(activeItem?.episodes || []).map((ep, idx) => (
                                            <div key={idx} className="p-6 bg-[#1c1f23] border border-slate-800 group hover:border-teal-500 transition-all flex flex-col gap-3 shadow-xl">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-[9px] text-teal-500 font-black tracking-widest uppercase">ACT {ep?.act}</span>
                                                    <button onClick={() => getBrief(activeItem.id, idx)} className="px-5 py-1.5 bg-slate-800 text-teal-500 text-[9px] font-black uppercase border border-slate-700 hover:bg-teal-500 hover:text-black transition-all">Run_Brief</button>
                                                </div>
                                    ...
                                    <div className="bg-black/40 border border-slate-800 rounded-xl p-10 h-[800px] overflow-y-auto shadow-2xl relative">
                                        <h4 className="text-teal-500 text-[10px] font-black uppercase mb-8 border-b border-slate-900 pb-4 flex items-center gap-3"><ShieldCheck size={18}/> Production_Brief</h4>
                                        ...
                                        )) : <div className="h-full flex items-center justify-center text-slate-800 text-[10px] font-black uppercase tracking-widest opacity-20">Select Node to Ground Brief</div>}
                                    </div>
                                </div>
                            </div>
                        )}
                        {/* PERSONA DETAIL (Same as v33.3) */}
                    </div>
                )}
            </section>
            ...
            <aside className="w-[400px] bg-[#0b0c0e] p-10 flex flex-col gap-12 border-l border-slate-800 overflow-y-auto shadow-2xl shrink-0">
                ...
                <div className="space-y-6">
                    ...
                    <div className="space-y-2"><p className="text-[9px] text-slate-600 uppercase font-black flex justify-between tracking-widest">Nodes <span>{newS.episodes_count}</span></p><input type="range" min="1" max="24" step="1" className="w-full accent-teal-500" value={newS.episodes_count} onChange={(e) => setNewS({...newS, episodes_count: e.target.value})} /></div>
