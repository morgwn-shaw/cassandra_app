...
const CONFIG = {
    ...
    D: ["Unresolved Sexual Tension", "Grudging Respect", "Bitter Rivals", "Mentor / Mentee", "Buddy Cop", "Frenemies", "Shared Trauma Bond"],
    T: ["Witnessed server farm bleed-out.", "Neural Betrayal.", "Identity Wiped.", "Exposed laundering ring."]
};

const App = () => {
    // FORCE INITIAL STATE AS EMPTY ARRAYS TO PREVENT .MAP ERRORS
    ...
    const [today, setToday] = useState('2026-03-24');
    ...
    const sync = async () => {
        ...
        } catch (e) { console.error("Signal dead."); }
    };
    useEffect(() => { sync(); }, []);
    ...
    const run = async (path, body, method = 'POST') => {
        ...
        } catch (e) { window.alert("TIMEOUT: Flash processing background signal."); } finally { setLoad(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[280px] border-r border-slate-800 bg-black/60 p-6 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-2 text-teal-500 font-black text-[11px] uppercase tracking-widest"><Cpu size={14}/> Apex_v98.0</div>
                    ...
                    <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-8">
                            <div className="space-y-6">
                                <div className="bg-teal-950/10 p-8 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[10px] font-black uppercase mb-3 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> 2026_Factual_Intelligence</h4><p className="text-lg text-slate-300 leading-relaxed uppercase">{active?.summary}</p></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-black/40 p-5 border border-slate-800 rounded-3xl"><h4 className="text-teal-500 text-[9px] font-black uppercase mb-3 flex items-center gap-1"><History size={12}/> Shared_Lore</h4>{active?.lore?.shared_anecdotes?.slice(0,5).map((a, j) => <p key={j} className="text-[9px] text-slate-500 italic mb-1 leading-tight">"{a}"</p>)}</div>
                                    <div className="bg-black/40 p-5 border border-slate-800 rounded-3xl"><h4 className="text-teal-400 text-[9px] font-black uppercase mb-3 flex items-center gap-1"><FastForward size={12}/> Future_Lore</h4><p className="text-[10px] text-slate-400 leading-relaxed italic">{active?.lore?.future_lore}</p></div>
                                </div>
                                <div className="space-y-3">
                                    {active?.episodes?.map((e, idx) => (
                                        <div key={idx} className="bg-[#1c1f23] p-5 border border-slate-800 rounded-xl flex justify-between items-center group hover:border-teal-500 transition-all shadow-lg">
                                            <div><span className="text-[8px] text-teal-600 font-black uppercase tracking-widest">Node {e.node || idx+1} - {e.phase}</span><h5 className="text-white font-bold italic uppercase leading-tight">{e.title}</h5></div>
                            ...
                            <div className="bg-black/40 p-8 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80"><h4 className="text-teal-500 text-[10px] font-black uppercase mb-6 flex items-center gap-2 italic underline underline-offset-8">Production_Intelligence</h4>{brief ? Object.entries(brief).map(([act, text], idx) => (
            ...
            <aside className="w-[380px] bg-[#0b0c0e] p-10 flex flex-col gap-10 border-l border-slate-800 overflow-y-auto shrink-0 shadow-2xl">
                ...
                <div className="space-y-6 border-t border-slate-900 pt-10"><h3 className="text-teal-400 text-[10px] font-black uppercase border-b border-slate-900 pb-3 tracking-widest"><Database size={16}/> Establish Season</h3>
                    ...
                    <button onClick={() => run('/season/reconcile', nS)} disabled={load || nS.host_ids.length !== 2} className="w-full py-5 bg-teal-500 text-black text-[10px] font-black uppercase shadow-2xl hover:bg-white active:scale-95 transition-all shadow-teal-500/20 italic tracking-widest">Establish Signal</button>
                </div>
            </aside>
        </div>
    );
