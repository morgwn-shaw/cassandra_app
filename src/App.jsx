...
// ATOMIC COMPONENT: Physically prevents brief errors from white-screening the main app
const SafeBrief = ({ data }) => {
    if (!data || typeof data !== 'object') return (
        <div className="h-64 flex flex-col items-center justify-center text-slate-700 text-[10px] font-black uppercase italic animate-pulse tracking-widest text-center">
            Select node to generate intelligence
        </div>
    );
    try {
        return Object.entries(data).map(([act, text], idx) => (
            <div key={idx} className="mb-6 border-l-2 border-teal-900/30 pl-4 animate-in slide-in-from-left">
                <span className="text-[9px] text-teal-700 font-black uppercase tracking-tighter">{act.replace('_', ' ')}</span>
                <p className="text-sm text-slate-400 italic leading-relaxed uppercase mt-1">"{text}"</p>
            </div>
        ));
    } catch (e) { return <p className="text-red-500 font-black">BRIEF_PARSE_ERROR</p>; }
};
...
const App = () => {
    ...
    // STABILITY LOOKUP: derive data from ID to prevent reference crashes
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v133.0</div>
                ...
                {!activeId ? (<div className="grid grid-cols-2 gap-8 animate-in fade-in">
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            ...
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80 shadow-teal-500/5"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline decoration-teal-900 underline-offset-8 tracking-widest">Forensic_Audit</h4>
