...
import { Zap, Trash2, Loader2, UserPlus, Wifi, Cpu, RefreshCw, X, Archive, BookOpen, Mic, Copy, Clock, Layers, Music, PlayCircle } from 'lucide-react';
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 custom-scrollbar shadow-2xl">
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <select className="bg-slate-900/50 p-4 border border-slate-800 text-teal-500 uppercase rounded-xl outline-none" value={nP.gender} onChange={e => setNP({...nP, gender: e.target.value})}>
                ...
                <div className="space-y-6 border-t border-slate-800 pt-8">
                    ...
                    <select className="w-full bg-slate-900/50 p-4 border border-slate-800 text-teal-500 uppercase rounded-xl outline-none" value={nS.relationship} onChange={e => setNS({...nS, relationship: e.target.value})}>
                    ...
                    <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 bg-black/20 rounded-xl custom-scrollbar">
                        {personas.map(p => (
                            <button 
                                key={p.id} 
                                onClick={() => { 
                                    const ids = nS.host_ids.includes(p.id) 
                                        ? nS.host_ids.filter(x => x !== p.id) 
                                        : [...nS.host_ids, p.id]; 
