...
import { Zap, Activity, Code, Cpu, Layers, Fingerprint, Trash2, Plus, PlayCircle, UserPlus, Database, Quote, X, RefreshCw } from 'lucide-react';
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        ...
        setLiveStatus(data);
        if (data.stage === 'SUCCESS') refreshData(); // Auto-pull data when engine finishes
      } catch (e) {}
    }, 1500);
    ...
    <div className="h-screen w-screen bg-[#020202] text-zinc-400 font-mono flex overflow-hidden">
      ...
      <section className="w-[450px] border-r border-zinc-900 bg-black flex flex-col p-6 shadow-2xl overflow-hidden">
        <div className="flex items-center justify-between mb-8">
            <div className="text-[#00ffcc] font-black uppercase text-[10px] tracking-[0.4em] flex items-center gap-2">
                <Zap size={14} className="animate-pulse" /> Shadow_Kernel_v24.2
            </div>
            <button onClick={refreshData} className="text-zinc-700 hover:text-[#00ffcc] transition-all"><RefreshCw size={14}/></button>
        </div>

        <div className="flex-1 flex flex-col gap-4 overflow-hidden">
            <div className="p-4 bg-zinc-950 border border-zinc-900 rounded">
                <p className="text-[#00ffcc] text-[9px] font-black uppercase mb-4 flex items-center gap-2 tracking-tighter"><Activity size={12}/> Kernel_History</p>
                <div className="h-56 overflow-y-auto space-y-1 pr-2 custom-scrollbar text-[10px]">
                    {liveStatus.history.map((log, i) => (
                        <div key={i} className={`border-l pl-3 py-1 font-bold ${log.includes('SUCCESS') ? 'text-[#00ffcc] border-[#00ffcc]' : log.includes('ERROR') ? 'text-red-500 border-red-500' : 'text-zinc-700 border-zinc-900'}`}>{log}</div>
                    ))}
                    <div ref={logEndRef} />
                </div>
            </div>
            ...
            <div className="p-4 bg-zinc-950 border border-zinc-900 rounded flex-1 flex flex-col overflow-hidden">
                ...
                <pre className="text-[9px] text-zinc-500 bg-black p-3 rounded flex-1 overflow-auto whitespace-pre-wrap leading-tight font-sans italic">{liveStatus.response_payload}</pre>
            </div>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {activeTab === 'season' && seasons.map((s, i) => (
                  <div key={i} className="bg-black border border-zinc-900 p-8 rounded relative group hover:border-[#00ffcc]/30 transition-all cursor-pointer" onClick={() => setActiveItem(s)}>
                      ...
                      <div className="w-full py-3 bg-zinc-900 text-[#00ffcc] text-[9px] font-black uppercase flex items-center justify-center gap-3 border border-zinc-800">Open_Vault</div>
                  </div>
              ))}
        ...
        {activeItem && activeTab === 'season' && (
            <div className="animate-in fade-in duration-500 space-y-12">
                <div className="flex justify-between items-center border-b border-zinc-900 pb-8">
                    ...
                    <button onClick={() => setActiveItem(null)} className="text-zinc-600 hover:text-white uppercase text-[10px] font-black flex items-center gap-2"><X size={16}/> Close_Vault</button>
                </div>
        ...
        {activeItem && activeTab === 'persona' && (
            <div className="animate-in fade-in duration-500 space-y-12">
                <div className="flex justify-between items-center border-b border-zinc-900 pb-8">
                    ...
                    <button onClick={() => setActiveItem(null)} className="text-zinc-600 hover:text-white uppercase text-[10px] font-black flex items-center gap-2"><X size={16}/> Exit_DNA</button>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="bg-black/40 border border-zinc-900 p-10 rounded-xl flex flex-col">
                        ...
                        <div className="text-[13px] text-zinc-400 leading-relaxed font-sans overflow-y-auto whitespace-pre-wrap">{activeItem.archive?.bio}</div>
                    </div>
                    <div className="bg-black/40 border border-zinc-900 p-10 rounded-xl flex flex-col">
                        ...
                        <div className="space-y-4 overflow-y-auto custom-scrollbar h-[500px]">
      ...
      <section className="w-[450px] bg-black p-10 flex flex-col gap-12 overflow-y-auto border-l border-zinc-900">
        ...
        <div className="space-y-6 border border-zinc-900 p-8 rounded-2xl bg-zinc-950/20">
            <h3 className="text-[#00ffcc] text-[10px] font-black uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><UserPlus size={14}/> Spawn_Identity</h3>
            <div className="space-y-4">
                <input className="w-full bg-zinc-900 p-4 border border-zinc-800 text-[11px] text-[#00ffcc] font-bold outline-none uppercase" placeholder="IDENTITY_NAME" value={newP.name} onChange={(e) => setNewP({...newP, name: e.target.value})} />
        ...
        {/* ESTABLISH ARC */}
        <div className="space-y-6 border border-zinc-900 p-8 rounded-2xl bg-zinc-950/20">
            <h3 className="text-[#00ffcc] text-[10px] font-black uppercase tracking-[0.2em] mb-4 flex items-center gap-2"><Database size={14}/> Establish_Arc</h3>
            <div className="space-y-8">
                ...
                <div className="space-y-3">
                    <p className="text-zinc-600 text-[9px] font-black uppercase ml-1 tracking-tighter">Identity_Pairing (Need 2)</p>
                ...
                <button 
                    ...
                    className={`w-full py-6 text-[11px] font-black uppercase tracking-widest transition-all rounded-xl ${loading || !newS.topic || newS.host_ids.length < 2 ? 'bg-zinc-950 text-zinc-800 opacity-20 border border-zinc-900' : 'bg-[#00ffcc] text-black hover:bg-white shadow-xl shadow-[#00ffcc]/10'}`}
                >
