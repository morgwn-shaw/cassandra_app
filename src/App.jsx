...
import { Trash2, Loader2, UserPlus, Wifi, Cpu, RefreshCw, X, Archive, Clock, Layers, Copy, PlayCircle } from 'lucide-react';
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px] select-none">
                ...
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-20 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full h-full border border-teal-900/40 rounded-3xl p-12 flex gap-12 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="w-[300px] shrink-0 space-y-6">
                            ...
                            <h2 className="text-4xl font-black text-white uppercase italic">{viewPersona?.name}</h2>
                            <div className="p-4 bg-red-950/20 border border-red-900/30 text-red-500 rounded-xl text-[9px] font-black italic">Trauma: {viewPersona?.trauma}</div>
                        </div>
                        <div className="flex-1 overflow-y-auto space-y-8 select-text">
                            <div className="bg-black/40 p-8 rounded-2xl border border-slate-800"><p className="text-slate-300 text-sm uppercase">{viewPersona?.archive?.bio}</p></div>
                        </div>
                    </div>
                </div>
            )}
            ...
            <main className="flex-1 flex flex-col p-10 bg-[#0d0f11] relative overflow-hidden">
                ...
                {!activeId ? (
                    <div className="grid grid-cols-2 gap-6 overflow-y-auto custom-scrollbar">
                        {seasons.map(s => (
                            <div key={s.id} onClick={() => setActiveId(s.id)} className="bg-[#1c1f23] p-8 border border-slate-800 rounded-3xl cursor-pointer hover:border-teal-500 relative h-fit shadow-xl">
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 custom-scrollbar shadow-2xl">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-8">
                    ...
                    <button onClick={() => run('/season/reconcile', nS)} disabled={nS.host_ids.length !== 2 || !nS.topic} className="w-full py-4 bg-teal-500 text-black font-black uppercase text-[10px] rounded-xl hover:bg-white transition-all shadow-teal-500/10">Establish Season</button>
                </div>
            </aside>
        </div>
    );
