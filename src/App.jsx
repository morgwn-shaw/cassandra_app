...
const App = () => {
    ...
    useEffect(() => { 
        sync();
        const interval = setInterval(sync, 10000); // Background sync every 10s
    ...
    const runProduction = async (epIdx) => {
        ...
        try {
            for (let i = 1; i <= 6; i++) {
                setStatus(`INITIATING ACT ${i}...`);
                
                // Trigger background thread on server
                await fetch(`${BASE_URL}/episode/act_script`, {
                    method: 'POST',
                    mode: 'cors',
                    ...
                    body: JSON.stringify({
                        ...
                        ep_idx: epIdx,
                        act_num: i,
                        previous_script: fullScript.slice(-800)
                    })
                });

                // Polling for the script to appear in the DB
                let actSaved = false;
                let attempts = 0;
                while (!actSaved && attempts < 30) { // 2 minute timeout cap
                    setStatus(`ACT ${i}: AI PRODUCING (WAITING FOR DB SYNC)...`);
                    await new Promise(r => setTimeout(r, 4000));
                    
                    const res = await fetch(`${BASE_URL}/season/list`);
                    const latestSeasons = await res.json();
                    const currentS = latestSeasons.find(x => x.id === activeId);
                    const epData = currentS?.episodes[epIdx];
                    
                    // Check if the server-side thread has saved the act
                    if (epData?.temp_scripts?.[i]) {
                        fullScript += `\n\n--- [ACT ${i}] ---\n\n` + epData.temp_scripts[i];
                        actSaved = true;
                    }
                    attempts++;
                }
                if (!actSaved) throw new Error(`Act ${i} timed out.`);
            }
            
            setStatus("COMPILING PRODUCTION SCRIPT...");
            await fetch(`${BASE_URL}/episode/save_full`, {
                method: 'POST',
                mode: 'cors',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ season_id: activeId, ep_idx: epIdx, full_script: fullScript })
            });
            await sync();
        } catch (e) {
    ...
    const createSeason = async () => {
        ...
        try {
            ...
            setStatus("GENERATING SHARED LORE...");
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px] select-none">
                    ...
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="w-[300px] shrink-0 space-y-6">
                            ...
                            <div className="space-y-4 pt-6 border-t border-slate-800 h-[320px] overflow-y-auto custom-scrollbar pr-3">
                                <div className="space-y-2">
                                    <h4 className="text-teal-500 font-black text-[9px] uppercase tracking-widest italic flex items-center gap-2">Preferences_Like</h4>
                                ...
                                <div className="space-y-2">
                                    <h4 className="text-red-500 font-black text-[9px] uppercase tracking-widest italic flex items-center gap-2">Aversions_Dislike</h4>
                                    <div className="flex flex-wrap gap-1">
                        ...
                        <div className="flex-1 overflow-y-auto space-y-8 select-text custom-scrollbar pr-6">
                            <div className="bg-black/40 p-8 rounded-2xl border border-slate-800 shadow-inner">
                                <h4 className="text-teal-500 font-black mb-4 uppercase flex items-center gap-2 italic">Behavioral_Bio</h4>
                                <p className="text-slate-300 text-sm leading-relaxed uppercase">{viewPersona?.archive?.bio}</p>
                            </div>
                            <div className="space-y-4">
                                <h4 className="text-teal-500 font-black uppercase flex items-center gap-2 italic">Life_Anecdotes</h4>
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="flex items-center gap-3 text-teal-500 font-black uppercase tracking-widest border-b border-teal-900/30 pb-4 italic"><Cpu size={18}/> Apex_v171.0</div>
                <button onClick={sync} className="w-full p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all italic flex items-center justify-center gap-2"><Wifi size={14}/> Sync Handshake</button>
            </aside>
                    ...
                    <div className="flex flex-col h-full gap-8 overflow-hidden">
                        {viewLore ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-3xl p-10 overflow-y-auto custom-scrollbar space-y-6">
                                <h3 className="text-teal-500 font-black uppercase mb-6 flex items-center gap-2 border-b border-teal-900/20 pb-4 text-xl tracking-[0.2em] italic">Shared History Log</h3>
                            ...
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-12 flex flex-col gap-8 overflow-hidden relative shadow-inner">
                                <button onClick={() => setActiveEp(null)} className="absolute top-8 right-12 text-slate-500 hover:text-white uppercase text-[10px] font-black italic tracking-[0.2em]">[ Back_to_Season ]</button>
                                <div className="space-y-3 border-b border-slate-800 pb-8">
                                    ...
                                    <p className="text-teal-500 uppercase font-black text-[11px] tracking-[0.3em] italic text-center">Sequential Production Engine | Act-by-Act Logic</p>
                                </div>
                                <div className="flex-1 overflow-y-auto custom-scrollbar pr-6 space-y-10">
                                    {activeSeason.episodes[activeEp].full_script ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full gap-8">
                                            ...
                                            <button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl hover:bg-white transition-all shadow-[0_0_30px_rgba(20,184,166,0.3)]">Launch Production Sequencer</button>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">
                                {activeSeason.episodes.map((e, idx) => (
                                    <div key={idx} onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500/50 cursor-pointer transition-all shadow-xl group h-fit">
                                        ...
                                        {e.full_script && <div className="mt-6 pt-4 border-t border-slate-800 text-teal-500 text-[10px] font-black uppercase italic flex items-center gap-2"><Zap size={12}/> Script_Finalized</div>}
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <div className="space-y-6 p-8 bg-black/40 rounded-[2rem] border border-slate-800">
                        <div className="space-y-3">
                            <div className="flex justify-between text-[10px] font-black uppercase italic text-slate-500"><span>Nodes: {nS.episodes_count}</span></div>
                            <input type="range" min="1" max="24" className="w-full accent-teal-500 h-1 bg-slate-800" value={nS.episodes_count} onChange={e => setNS({...nS, episodes_count: parseInt(e.target.value)})} />
                        </div>
                        <div className="space-y-3">
                            <div className="flex justify-between text-[10px] font-black uppercase italic text-slate-500"><span>Runtime Gravity: {nS.target_runtime}m</span></div>
                            <input type="range" min="5" max="60" step="5" className="w-full accent-teal-500 h-1 bg-slate-800" value={nS.target_runtime} onChange={e => setNS({...nS, target_runtime: parseInt(e.target.value)})} />
                        </div>
                    </div>
