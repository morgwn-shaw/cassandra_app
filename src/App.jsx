...
import { Trash2, Loader2, UserPlus, Wifi, Cpu, RefreshCw, X, Archive, Clock, Layers, Mic, PlayCircle, BookOpen } from 'lucide-react';
                    ...
                    <div className="flex flex-col h-full gap-6">
                        <div className="flex flex-wrap gap-2 overflow-y-auto max-h-[140px] custom-scrollbar p-3 border border-slate-900 rounded-2xl bg-black/20 shrink-0">
                            {activeSeason?.episodes?.map((e, idx) => (
                                <button key={idx} onClick={() => setActiveEpIdx(idx)} className={`px-5 py-3 border text-[10px] font-black uppercase rounded-xl transition-all ${activeEpIdx === idx ? 'bg-teal-500 text-black border-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-400'}`}>Node_{e.node}: {e.title}</button>
                            ))}
                        </div>
                    </div>
                )}
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 custom-scrollbar shadow-2xl">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-8">
                    ...
                    <div className="space-y-4 bg-black/40 p-4 rounded-xl border border-slate-800">
                        ...
                        <input type="range" min="1" max="24" className="w-full accent-teal-500 h-1 bg-slate-800 appearance-none rounded-lg cursor-pointer" value={nS.episodes_count} onChange={e => setNS({...nS, episodes_count: parseInt(e.target.value)})} />
                        <input type="range" min="5" max="20" className="w-full accent-teal-500 h-1 bg-slate-800 appearance-none rounded-lg cursor-pointer" value={nS.target_runtime} onChange={e => setNS({...nS, target_runtime: parseInt(e.target.value)})} />
                    </div>
