...
import { Zap, Trash2, Database, Quote, Loader2, UserPlus, History, FastForward, Wifi, Cpu, ThumbsUp, ThumbsDown, Dice5, ScrollText, PlayCircle, Globe, Calendar, RefreshCcw } from 'lucide-react';
...
const App = () => {
    ...
    const [load, setLoad] = useState(false); const [eng, setEng] = useState('SCAVENGING...');
    const [brief, setBrief] = useState(null); const [today, setToday] = useState('CALIBRATING...');
    ...
    const sync = async () => {
        ...
        } catch (e) { console.error("Vault link down."); }
    };
    useEffect(() => { sync(); }, []);
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.model?.toUpperCase() || 'OFFLINE'); setToday(d.date);
    ...
    const run = async (path, body, method = 'POST') => {
        ...
        } catch (e) { window.alert("SIGNAL TIMEOUT: background pass in progress. Refresh in 60s."); } finally { setLoad(false); }
    };

    // Defensive render lists
    const s_list = Array.isArray(seasons) ? seasons : [];
    const p_list = Array.isArray(personas) ? personas : [];
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v101.0</div>
                ...
                <div className="space-y-2">
                    ...
                    <button onClick={() => setLoad(false)} className="w-full p-2 text-[9px] text-teal-900 font-black uppercase hover:text-teal-400 flex items-center justify-center gap-2"><RefreshCcw size={12}/> Reset Loading State</button>
                </div>
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-900"><button onClick={() => run('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black border border-red-900/30 hover:bg-red-600 transition-all uppercase"><Zap size={14}/> Purge Vault</button></div>
            </aside>

            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Establishing Flash Context...</p></div>}
                <div className="flex gap-4 mb-10 border-b border-slate-800 pb-8"><button onClick={() => {setActive(null); setTab('season');}} className={`px-12 py-4 text-[11px] font-black ${tab === 'season' && !active ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>SEASONS</button><button onClick={() => {setActive(null); setTab('persona');}} className={`px-12 py-4 text-[11px] font-black ${tab === 'persona' && !active ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>PERSONAS</button></div>
                
                {!active ? (<div className="grid grid-cols-2 gap-8">
                    {(tab === 'season' ? s_list : p_list).map((i, k) => (
                        <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all shadow-inner" onClick={() => setActive(i)}>
                            ...
                            <img src={i?.portrait} className="w-20 h-20 rounded-2xl grayscale group-hover:grayscale-0 transition-all shadow-xl" alt="D"/>
                            <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-xl italic truncate leading-none tracking-tighter">{i?.title || i?.name}</h4><p className="text-[10px] text-teal-500 font-black opacity-60 uppercase mt-1 italic">{i?.rel || i?.role}</p></div>
                        </div>
                    ))}
                </div>) : (
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-10"><h2 className="text-7xl font-black text-white uppercase italic tracking-tighter leading-none shadow-teal-500/5">{active?.title || active?.name}</h2><button onClick={() => setActive(null)} className="bg-teal-500 text-black px-12 py-4 text-[11px] font-black shadow-2xl active:scale-90">BACK</button></div>
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> 2026_Forensic_Summary</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active?.summary}</p></div>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="bg-black/40 p-6 border border-slate-800 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[9px] font-black uppercase mb-4 flex items-center gap-2"><FastForward size={14}/> Future_Lore</h4><p className="text-[11px] text-slate-400 leading-relaxed italic">{active?.lore?.future_lore}</p></div>
                                </div>
                                <div className="space-y-4">
                                    {active?.episodes?.map((e, idx) => (
                                        <div key={idx} className="bg-[#1c1f23] p-6 border border-slate-800 rounded-2xl flex justify-between items-center group hover:border-teal-500 transition-all shadow-lg shadow-black/50">
                                            <div><span className="text-[9px] text-teal-600 font-black uppercase tracking-widest">Node {idx+1}</span><h5 className="text-white font-bold italic uppercase leading-tight">{e.title}</h5></div>
                            ...
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline decoration-teal-900 underline-offset-8 tracking-widest">Episode_Brief</h4>{brief ? Object.entries(brief).map(([act, text], idx) => (
