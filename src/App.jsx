...
import { Zap, Activity, Trash2, Database, Quote, Brain, Loader2, Fingerprint, ScrollText, ShieldCheck, Dice5, UserPlus, ShieldAlert, ChevronRight, PlayCircle, MessageSquare, History, FastForward } from 'lucide-react';
...
const App = () => {
    ...
    const getBrief = async (seasonId, idx) => {
        ...
        try {
            ...
            console.error("Briefing crash:", e);
        } finally { setLoading(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0a0c0e] text-slate-400">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Zap size={16} /> Telemetry_v44.0</div>
                ...
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-900">
                    ...
                    <button onClick={() => runAction('/purge', {})} className="w-full p-4 bg-red-950/20 border border-red-900/30 text-red-500 text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition-all shadow-2xl flex items-center justify-center gap-2">
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416] min-w-[800px]">
                {loading && (
                    <div className="absolute inset-0 bg-black/80 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase tracking-[0.8em] animate-pulse italic">Syncing_Apex_Signals...</p>
                    </div>
                )}
                ...
                {!activeItem ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                        {((activeTab === 'season' ? seasons : personas) || []).map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-10 rounded-2xl cursor-pointer hover:border-teal-500 transition-all flex gap-8 items-center shadow-xl group" onClick={() => setActiveItem(item)}>
                                ...
                                {viewMode === 'god' && (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                        ...
                                        <div className="bg-[#1c1f23] p-10 border border-slate-800 rounded-2xl shadow-xl">
                                            ...
                                            <p className="text-[14px] text-slate-400 uppercase leading-relaxed font-sans">{activeItem?.lore?.future_lore || "No roadmap found."}</p>
                                        </div>
                                    </div>
                                )}
                                    ...
                                    <div className="bg-black/40 border border-slate-800 rounded-3xl p-12 h-[850px] overflow-y-auto shadow-2xl relative">
                                        <div className="flex justify-between items-center mb-10 border-b border-slate-900 pb-6">
                                            <div className="flex flex-col">
                                                ...
                                                <span className="text-[9px] text-teal-900 uppercase font-black italic">{activeBrief?.archetype} Structure</span>
                                            </div>
