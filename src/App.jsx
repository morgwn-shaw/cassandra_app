...
import { Trash2, Loader2, UserPlus, Wifi, Cpu, RefreshCw, X, Archive, Clock, Layers, Copy, Mic, PlayCircle, BookOpen } from 'lucide-react';
...
const CONFIG = {
    ...
    T: ["Neural Betrayal.", "Identity Wiped.", "None", "Witnessed server farm bleed-out."]
};
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px] select-none">
            {viewPersona && (
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-20 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full h-full border border-teal-900/40 rounded-3xl p-12 flex gap-12 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-8 select-text">
                    ...
                    <div className="flex flex-col h-full gap-6 overflow-hidden">
                        <div className="flex flex-wrap gap-2 overflow-y-auto max-h-[140px] custom-scrollbar p-3 border border-slate-900 rounded-2xl bg-black/20 shrink-0">
                            {activeSeason?.episodes?.map((e, idx) => (
                                <button key={idx} onClick={() => setActiveEpIdx(idx)} className={`px-5 py-3 border text-[10px] font-black uppercase rounded-xl min-w-[160px] transition-all ${activeEpIdx === idx ? 'bg-teal-500 text-black border-white shadow-lg' : 'bg-slate-900 border-slate-800 text-slate-400 hover:border-teal-500'}`}>Node_{e.node}: {e.title}</button>
                            ))}
                        </div>

                        {activeEp && (
                            <div className="flex-1 bg-black border border-teal-900/30 p-10 rounded-[40px] overflow-auto shadow-2xl relative custom-scrollbar select-text">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {terminalMode === 'brief' && activeEp.saved_brief && (
                                            <div className="flex gap-1 ml-4 border-l border-slate-800 pl-4">
                                                {[1,2,3,4,5,6].map(n => (
                                                    <button key={n} onClick={() => run('/showrunner/script', {title: activeEp.title, season_id: activeId, act_num: n, context: activeEp.saved_brief})} className={`px-3 py-1 rounded text-[8px] font-black transition-all ${activeEp.acts?.[n] ? 'bg-teal-600 text-black' : 'bg-slate-800'}`}>Act_{n}</button>
                                                ))}
                                            </div>
                                        )}
                                        {terminalMode === 'script' && <button onClick={() => { navigator.clipboard.writeText(JSON.stringify(getMasterScript())); window.alert("JSON_COPIED"); }} className="bg-teal-600 text-black px-6 py-2 rounded-full text-[9px] font-black uppercase flex items-center gap-2"><Copy size={14}/> Copy JSON</button>}
                                    </div>
                                </div>
                                <div>
                                    ...
                                    {terminalMode === 'script' && (
                                        <div className="space-y-6">
                                            {getMasterScript().map((line, idx) => (
                                                <div key={idx} className="border-l-2 border-teal-900/40 pl-6"><span className="text-[9px] font-black text-teal-700 uppercase italic">[{line.voice_id}] {line.name}</span><p className="text-slate-300 text-sm leading-relaxed mt-1 uppercase">"{line.content}"</p></div>
                                            ))}
                                        </div>
                                    )}
                                </div>
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 custom-scrollbar shadow-2xl">
                {/* PERSONA GENERATOR (RESTORED) */}
                <div className="space-y-6">
                    <h3 className="text-teal-500 text-[11px] font-black uppercase flex items-center gap-2 tracking-[0.2em]"><UserPlus size={18}/> Identity Spawn</h3>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <select className="bg-slate-900 p-4 border border-slate-800 text-teal-500 rounded-xl outline-none" value={nP.gender} onChange={e => setNP({...nP, gender: e.target.value})}>{CONFIG.G.map(g => <option key={g} value={g}>{g}</option>)}</select>
                        <select className="bg-slate-900 p-4 border border-slate-800 text-teal-500 rounded-xl outline-none" value={nP.trauma} onChange={e => setNP({...nP, trauma: e.target.value})}>{CONFIG.T.map(t => <option key={t} value={t}>{t}</option>)}</select>
                    </div>
                    ...
                    <button onClick={() => run('/persona/create', nP)} disabled={!nP.name} className="w-full py-4 bg-teal-500 text-black font-black uppercase text-[10px] rounded-xl hover:bg-white transition-all shadow-teal-500/10">Commit DNA</button>
                ...
                {/* SEASON GENERATOR (RESTORED) */}
                <div className="space-y-6 border-t border-slate-800 pt-8">
                    <h3 className="text-teal-500 text-[11px] font-black uppercase flex items-center gap-2 tracking-[0.2em]"><Archive size={18}/> Establish Season</h3>
                    <input className="w-full bg-slate-900 p-4 border border-slate-800 text-white font-bold rounded-xl outline-none focus:border-teal-500 uppercase" placeholder="TOPIC" value={nS.topic} onChange={e => setNS({...nS, topic: e.target.value})} />
                    <select className="w-full bg-slate-900 p-4 border border-slate-800 text-teal-500 rounded-xl outline-none" value={nS.relationship} onChange={e => setNS({...nS, relationship: e.target.value})}>{CONFIG.D.map(d => <option key={d} value={d}>{d}</option>)}</select>
                    ...
                    <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 bg-black/20 rounded-xl custom-scrollbar border border-slate-800">
                        {personas.map(p => (
                            ...
                            }} className={`p-2 text-[8px] font-black border truncate uppercase rounded-md transition-all ${nS.host_ids.includes(p.id) ? 'border-teal-500 text-teal-400 bg-teal-500/10' : 'border-slate-800 text-slate-500 hover:border-teal-900'}`}>{p.name}</button>
                        ))}
                    </div>
