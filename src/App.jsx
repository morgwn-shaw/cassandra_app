...
import { Zap, Trash2, Database, Quote, Loader2, UserPlus, History, FastForward, Wifi, Cpu, ThumbsUp, ThumbsDown, Dice5, ScrollText, PlayCircle, Globe, ZapOff } from 'lucide-react';
...
const App = () => {
    // Defensive Initialization: Never null, always arrays or strings
    ...
    const [load, setLoad] = useState(false); const [eng, setEng] = useState('FLASH_CONTEXT_ACTIVE');
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.model?.toUpperCase() || 'ERROR');
            window.alert("FLASH HANDSHAKE SUCCESS");
    ...
    const run = async (path, body, method = 'POST') => {
        ...
        } catch (e) { window.alert("TIMEOUT: Server still processing in background. Refresh in 60s."); } finally { setLoad(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v93.0</div>
                    <div className="text-[9px] text-teal-900 font-black mt-1 italic uppercase tracking-widest flex items-center gap-2"><Zap size={10}/> {eng}</div>
                </div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Compiling 2026 Context...</p></div>}
                
                <div className="flex gap-4 mb-10 border-b border-slate-800 pb-8">
                    <button onClick={() => {setActive(null); setTab('season'); setBrief(null);}} className={`px-12 py-4 text-[11px] font-black transition-all ${tab === 'season' && !active ? 'bg-teal-500 text-black' : 'bg-slate-800'}`}>SEASONS</button>
                    <button onClick={() => {setActive(null); setTab('persona'); setBrief(null);}} className={`px-12 py-4 text-[11px] font-black transition-all ${tab === 'persona' && !active ? 'bg-teal-500 text-black' : 'bg-slate-800'}`}>DNA_VAULT</button>
                </div>
                ...
                {!active ? (<div className="grid grid-cols-2 gap-8 animate-in fade-in">
                    {(tab === 'season' ? seasons : personas).map((i, k) => (
                        <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all shadow-inner" onClick={() => setActive(i)}>
                            ...
                            <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-xl italic truncate tracking-tighter">{i.title || i.name}</h4><p className="text-[10px] text-teal-500 font-black opacity-60 uppercase mt-1 italic">{i.rel || i.role}</p></div>
                        </div>
                    ))}
                </div>) : (
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            ...
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline underline-offset-8">Production_Intelligence</h4>{brief ? Object.entries(brief).map(([act, text], idx) => (
