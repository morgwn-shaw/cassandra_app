...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, Terminal, Mic2, Headphones, Sparkles, UserCircle, HeartPulse, ShieldAlert } from 'lucide-react';
...
const CONFIG = {
    ...
    D: ["Unresolved Sexual Tension", "Mentor / Mentee", "Enemies", "Frenemies", "Grudging Respect", "Buddy Cop", "Bitter Rivals", "Strategic Alliance", "Estranged Family"],
};
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V184_RECOVERY", type: "system" }]);
    ...
    const createSeason = async () => {
        ...
        addLog(`SEASON_RECONCILE: ${nS.topic}`);
        try {
            ...
            addLog(`ARCHETYPE_LOCKED: ${s.archetype}`, "system");
            
            setStatus("Generating Lore...");
            addLog(`LORE_GEN_START: Host Pairing...`);
            const r2 = await fetch(`${BASE_URL}/season/lore`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ season_id: s.id }) });
            const l = await r2.json();
            addLog(`LORE_GEN_COMPLETE: ${l.shared_history?.length} items`, "llm");
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0">
                ...
                <div ref={logRef} className="flex-1 overflow-y-auto p-4 space-y-2 bg-black/20 custom-scrollbar text-[9px] uppercase">
                    {logs.map((log, i) => (
                        <div key={i} className={`p-2 border rounded ${log.type === 'error' ? 'border-red-900/50 text-red-500' : log.type === 'llm' ? 'border-teal-900/50 text-teal-500' : 'border-slate-800'}`}>
                            [{log.t}] {log.m.substring(0, 60)}
                        </div>
                    ))}
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    ...
                    {activeId && (
                        <>
                            <div className="flex-1 text-teal-500 font-black uppercase italic truncate text-lg tracking-widest px-4"><ChevronRight className="inline mr-2"/>{activeSeason?.title || "Establishment..."}</div>
                            <button onClick={() => setViewLore(!viewLore)} className={`p-4 rounded-xl border flex items-center gap-2 uppercase font-black transition-all ${viewLore ? 'bg-teal-500 text-black border-teal-500' : 'border-slate-800 text-slate-500'}`}><History size={16}/> View Lore</button>
                        </>
                    )}
                </div>
            ...
            <aside className="w-[340px] bg-black/60 p-8 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <div className="grid grid-cols-2 gap-3 max-h-40 overflow-y-auto p-4 bg-black/20 rounded-2xl border border-slate-800">
                        {personas.map(p => (
                            <button key={p.id} onClick={() => { const ids = nS.host_ids.includes(p.id) ? nS.host_ids.filter(x => x !== p.id) : [...nS.host_ids, p.id]; setNS({...nS, host_ids: ids.slice(0, 2)}); }} className={`p-4 text-[9px] font-black border truncate uppercase rounded-2xl transition-all ${nS.host_ids.includes(p.id) ? 'border-teal-500 text-teal-400 bg-teal-500/10 shadow-[0_0_15px_rgba(20,184,166,0.1)]' : 'border-slate-800 text-slate-600 hover:border-slate-700'}`}>{p.name}</button>
                        ))}
                    </div>
