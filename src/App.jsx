...
const CONFIG = {
    ...
    TRAUMAS: ["Witnessed server farm bleed-out.", "Neural-link betrayal.", "Identity wiped by ghost protocol.", "Exposed data laundering.", "Escaped digital cult."]
};
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0d0f11] text-slate-400">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-xs uppercase tracking-widest"><Zap size={16} /> Telemetry_v36.5</div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {loading && (
                    <div className="absolute inset-0 bg-black/80 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase tracking-[0.8em] animate-pulse italic">Commiting_Data_to_Vault...</p>
                    </div>
                )}
                ...
                <div className="flex gap-4 mb-12 border-b border-slate-800 pb-8">
                    ...
                    <button onClick={() => {setActiveItem(null); setActiveBrief(null); setActiveTab('persona');}} className={`px-12 py-4 text-[11px] font-black transition-all ${activeTab === 'persona' && !activeItem ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800 text-slate-500'}`}>PERSONAS</button>
                </div>
                ...
                {!activeItem ? (
                    ...
                    <div className="animate-in slide-in-from-bottom-6 space-y-12">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-10">
                            ...
                            <button onClick={() => {setActiveItem(null); setActiveBrief(null);}} className="bg-teal-500 text-black px-10 py-4 text-xs font-black shadow-2xl">RETURN_TO_VAULT</button>
                        </div>

                        {activeTab === 'season' && (
                            <div className="flex flex-col gap-12">
                                <div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-2xl shadow-2xl">
                                    <h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 flex items-center gap-3 italic tracking-[0.3em]"><ScrollText size={20}/> 3_Act_Investigation_Blueprint</h4>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                                    ...
                                    <div className="bg-black/40 border border-slate-800 rounded-2xl p-12 h-[850px] overflow-y-auto shadow-2xl">
                                        ...
                                        )) : <div className="h-full flex items-center justify-center text-slate-800 text-xs font-black uppercase tracking-[0.4em] opacity-30 text-center italic">Awaiting_Node_Establishment</div>}
                                    </div>
                                </div>
                            </div>
                        )}
                        {/* PERSONA VIEW - SAME AS v33.3 */}
                    </div>
                )}
            </main>
