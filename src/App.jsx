...
const App = () => {
    ...
    const testPing = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.status); window.alert("HANDSHAKE SUCCESS");
    ...
    const runAction = async (path, body) => {
        ...
        try {
            ...
            if (d.error) window.alert("AI ERROR: " + d.error); 
            await sync();
        } catch (e) { 
            window.alert("SIGNAL ERROR: Check PythonAnywhere Error Logs."); 
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4"><div className="flex items-center gap-3 text-teal-500 font-black text-xs uppercase"><Cpu size={16}/> Apex_v80.0</div></div>
                <button onClick={testPing} className="p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all shadow-lg"><Wifi size={14}/> Handshake</button>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-xl"><Loader2 className="animate-spin text-teal-500 mb-2"/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest">Processing 2026 Data...</p></div>}
                ...
                {!active ? (<div className="grid grid-cols-2 gap-8 animate-in fade-in">
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="space-y-12"><div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-3xl"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 italic flex items-center gap-2"><ScrollText size={16}/> Grounded Briefing</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>{view === 'god' && <div className="grid grid-cols-2 gap-8"><div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-[400px] overflow-y-auto custom-scrollbar"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><History size={16}/> Shared History</h4>{(active.lore?.shared_anecdotes || []).map((a, j) => <p key={j} className="text-[11px] text-slate-500 italic mb-4 leading-tight">"{a}"</p>)}</div><div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-[400px] overflow-y-auto custom-scrollbar"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><FastForward size={16}/> Roadmap</h4><p className="text-sm text-slate-400 leading-relaxed">{active.lore?.future_lore}</p></div></div>}</div>)}
