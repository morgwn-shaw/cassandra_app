...
const CONFIG = {
    ...
    D: ["Unresolved Sexual Tension", "Grudging Respect", "Bitter Rivals", "Mentor / Mentee", "Buddy Cop", "Frenemies", "Strategic Alliance", "Shared Trauma Bond", "Competitive Peers", "Professional Disdain", "Master / Apprentice", "Reluctant Partners"],
...
const App = () => {
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.status); window.alert("HANDSHAKE: ONLINE");
        } catch (e) { window.alert("HANDSHAKE FAILED: Reload PythonAnywhere Web Tab."); } finally { setLoad(false); }
    };
    ...
    const run = async (path, body) => {
        setLoad(true); try {
            ...
            const d = await r.json(); if (d.error) window.alert("ERROR: " + d.error); await sync();
        } catch (e) { window.alert("FETCH FAILED: Check Connection."); } finally { setLoad(false); }
    };
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-6"><div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v71.0</div><div className="text-[10px] text-teal-900 uppercase font-black mt-2 tracking-tighter">{eng}</div></div>
                ...
                <div className="mt-auto space-y-4">
                    ...
                    <button onClick={() => run('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black uppercase border border-red-900/30 hover:bg-red-600">World_Reset</button>
                </div>
            </aside>
                    ...
                    <div className="grid grid-cols-2 gap-8 animate-in fade-in">
                        {((tab === 'season' ? seasons : personas) || []).map((i, k) => (
                            <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all" onClick={() => setActive(i)}>
                                ...
                                <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-xl italic truncate tracking-tighter leading-none">{i.title || i.name}</h4><p className="text-[10px] text-teal-500 font-black opacity-60 uppercase mt-2">{i.rel || i.role || 'DNA Fixed'}</p></div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (
                            <div className="space-y-12">
                                <div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-3xl"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><ScrollText size={16}/> Investigation_Brief</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>
