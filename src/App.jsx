...
const SafeBrief = ({ data }) => {
    ...
    return Object.entries(data).map(([act, text], idx) => (
        <div key={idx} className="mb-6 border-l-2 border-teal-900/30 pl-4 animate-in slide-in-from-left">
            <span className="text-[9px] text-teal-700 font-black uppercase tracking-widest">{act.replace('_', ' ')}</span>
            <p className="text-sm text-slate-400 italic leading-relaxed uppercase mt-1">"{text}"</p>
        </div>
    ));
};
...
const App = () => {
    ...
    const run = async (path, body, method = 'POST', skipSync = false) => {
        setLoad(true); try {
            ...
            if (!r.ok || d.error) throw new Error(d.error || "Signal Interrupted");
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> Forensic_Intelligence</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active?.summary || "Initialising forensic report..."}</p></div>
                            ...
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80 shadow-teal-500/5"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline decoration-teal-900 underline-offset-8 tracking-widest">Grounded_Audit</h4>
                                {brief ? <SafeBrief data={brief} /> : <div className="h-64 flex flex-col items-center justify-center text-slate-700 text-[10px] font-black uppercase italic animate-pulse tracking-widest text-center">Select node to generate intelligence</div>}
                            </div>
                        </div>)}
