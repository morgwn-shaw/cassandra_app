...
const App = () => {
    ...
    const runAction = async (endpoint, body) => {
        ...
        try {
            ...
            if (result.error) window.alert(`LOG_ERROR: ${result.error}`);
            await refreshData();
        } catch (e) { window.alert(`Connection Fault: ${e.message}`); }
    ...
    return (
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0d0f11] text-slate-400">
            ...
            {/* COLUMN 1: TELEMETRY (ENHANCED ERRORS) */}
            <aside className="w-[300px] border-r border-slate-800 bg-black/40 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-xs uppercase tracking-widest"><Zap size={16} /> Telemetry_v37.0</div>
                ...
                {!activeItem ? (
                        ...
                        {(activeTab === 'season' ? seasons : personas).map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-xl cursor-pointer hover:border-teal-500/50 transition-all flex gap-8 items-center shadow-xl group" onClick={() => setActiveItem(item)}>
                                <div className="w-16 h-16 rounded bg-slate-900 overflow-hidden border border-slate-800 shrink-0">
                                    {item?.portrait ? <img src={item.portrait} className="w-full h-full object-cover grayscale group-hover:grayscale-0" alt="DNA" /> : <Fingerprint className="m-4 text-slate-700" size={32}/>}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <h4 className="text-white font-black uppercase text-lg italic tracking-tighter truncate">{item?.title || item?.name || "MISSING_DNA"}</h4>
                    ...
                    <div className="animate-in slide-in-from-bottom-6 duration-500 space-y-12">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-10">
                            ...
                            <button onClick={() => {setActiveItem(null); setActiveBrief(null);}} className="bg-teal-500 text-black px-10 py-4 text-xs font-black">RETURN</button>
                        </div>
                        ...
                        {activeTab === 'season' && (
                            <div className="flex flex-col gap-12">
                                <div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-2xl shadow-2xl">
                                    ...
                                    <p className="text-[19px] text-slate-300 font-sans leading-relaxed uppercase">{activeItem?.summary || "DNA_SEQUENCING_FAILURE: PURGE AND RETRY."}</p>
                                </div>
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                                        ...
                                        {(activeItem?.episodes || []).map((ep, idx) => (
                                            <div key={idx} className="p-8 bg-[#1c1f23] border border-slate-800 group hover:border-teal-500 transition-all flex flex-col gap-4 shadow-xl rounded-lg">
                                                <div className="flex justify-between items-center">
                                                    <span className="text-[10px] text-teal-500 font-black uppercase">ACT {ep?.act}</span>
                                    ...
                                    <div className="bg-black/40 border border-slate-800 rounded-2xl p-12 h-[850px] overflow-y-auto shadow-2xl relative">
                                        <h4 className="text-teal-500 text-[12px] font-black uppercase mb-10 border-b border-slate-900 pb-6 flex items-center gap-4"><ShieldCheck size={24}/> Technical_Brief</h4>
                        ...
                        {activeTab === 'persona' && (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                <div className="bg-black/40 p-10 border border-slate-800 rounded shadow-2xl h-[600px] overflow-y-auto custom-scrollbar">
                                    ...
                                    <p className="text-[14px] text-slate-400 font-sans leading-relaxed whitespace-pre-wrap uppercase">{activeItem?.archive?.bio || "CORRUPTED_SIGNAL: NO BIO DATA FOUND."}</p>
                                </div>
                                <div className="bg-black/40 p-10 border border-slate-800 rounded shadow-2xl h-[600px] overflow-y-auto custom-scrollbar">
            ...
            <aside className="w-[420px] bg-[#0b0c0e] p-12 flex flex-col gap-14 border-l border-slate-800 overflow-y-auto shadow-2xl shrink-0">
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <select className="bg-[#1c1f23] p-5 border border-slate-800 text-xs text-teal-500 font-black outline-none" value={newP.gender} onChange={(e) => setNewP({...newP, gender: e.target.value})}>{["Male", "Female", "Non-Binary", "Gender-Fluid"].map(g => <option key={g} value={g}>{g}</option>)}</select>
                ...
                <div className="space-y-8 border-t border-slate-900 pt-14">
                    ...
                    <select className="w-full bg-[#1c1f23] p-5 border border-slate-800 text-xs text-teal-400 font-bold" value={newS.relationship} onChange={(e) => setNewS({...newS, relationship: e.target.value})}>{CONFIG.DYNAMICS.map(d => <option key={d}>{d}</option>)}</select>
                    ...
                    <div className="space-y-4">
                        ...
                        <input type="range" min="1" max="24" step="1" className="w-full accent-teal-500" value={newS.episodes_count} onChange={(e) => setNewS({...newS, episodes_count: e.target.value})} />
                    </div>
