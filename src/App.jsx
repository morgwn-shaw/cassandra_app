...
import { Zap, Activity, Code, Cpu, Layers, Fingerprint, Trash2, Plus, PlayCircle, UserPlus, Database, Quote, X, ShieldCheck, Send } from 'lucide-react';
...
const DYNAMICS = ["Unresolved Sexual Tension (UST)", "Grudging Mutual Respect", "Mentor / Protégé (Estranged)", "Bitter Professional Rivals", "Two Strangers sharing a Secret"];
      ...
      <section className="w-[420px] border-r border-slate-800 bg-[#08090a] flex flex-col p-6 shadow-2xl">
        <div className="flex items-center justify-between mb-8 border-b border-slate-800 pb-4">
            <div className="text-teal-400 font-black uppercase text-[10px] tracking-[0.4em] flex items-center gap-2">
                <Zap size={14} className={loading ? "animate-pulse text-amber-500" : ""} /> Kernel_v26.2
            </div>
            <div className="text-[8px] text-slate-600 font-bold uppercase">{liveStatus.active_engine?.split('/')[1]}</div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in duration-500">
              {activeTab === 'season' && seasons.map((s, i) => (
                  <div key={i} className="bg-[#161a1d] border border-slate-800 p-8 rounded-lg shadow-xl hover:border-teal-500/40 cursor-pointer relative" onClick={() => setActiveItem(s)}>
                      <button onClick={(e) => { e.stopPropagation(); fetch(`${API_BASE}/season/delete/${s.id}`, {method:'DELETE'}); refreshData(); }} className="absolute top-6 right-6 text-slate-700 hover:text-red-500"><Trash2 size={18}/></button>
                      ...
                      <div className="w-full py-3 bg-slate-800 text-teal-400 text-[9px] font-black uppercase flex items-center justify-center border border-slate-700">Open_Blueprint</div>
                  </div>
              ))}
              {activeTab === 'persona' && personas.map((p, i) => (
                  <div key={i} className="bg-[#161a1d] border border-slate-800 p-8 rounded-lg shadow-xl hover:border-teal-500/40 cursor-pointer relative" onClick={() => setActiveItem(p)}>
                      <button onClick={(e) => { e.stopPropagation(); fetch(`${API_BASE}/persona/delete/${p.id}`, {method:'DELETE'}); refreshData(); }} className="absolute top-6 right-6 text-slate-700 hover:text-red-500"><Trash2 size={18}/></button>
        ...
        {activeItem && activeTab === 'season' && (
            <div className="animate-in fade-in space-y-12">
                <div className="flex justify-between items-end border-b border-slate-800 pb-8">
                    ...
                    <button onClick={() => { setActiveItem(null); setActiveBrief(null); }} className="bg-teal-500 text-black px-8 py-3 text-[10px] font-black uppercase hover:bg-white transition-all shadow-xl">Return</button>
                </div>
                
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="space-y-6 bg-black/20 p-8 border border-slate-800 rounded-xl">
                        <h4 className="text-teal-500 text-[10px] font-black uppercase mb-4 italic border-b border-slate-800 pb-2">Nodes</h4>
                    ...
                    <div className="space-y-6 bg-black/20 p-8 border border-slate-800 rounded-xl">
                        <h4 className="text-teal-500 text-[10px] font-black uppercase mb-4 italic border-b border-slate-800 pb-2">6-Act_Brief</h4>
                        {activeBrief ? (
                           ...
                           <div className="h-64 flex items-center justify-center border border-dashed border-slate-800 rounded text-slate-700 text-[9px] font-black uppercase tracking-[0.4em]">Initialize_Node</div>
                        )}
                    </div>
                </div>
            </div>
        ...
        {activeItem && activeTab === 'persona' && (
            <div className="animate-in fade-in space-y-12">
                <div className="flex justify-between items-end border-b border-slate-800 pb-8">
                    ...
                    <button onClick={() => setActiveItem(null)} className="bg-teal-500 text-black px-8 py-3 text-[10px] font-black uppercase hover:bg-white transition-all shadow-xl">Exit</button>
                </div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    ...
                    <div className="bg-black/20 p-10 border border-slate-800 rounded-xl space-y-8 h-[600px] overflow-y-auto pr-4 custom-scrollbar">
                        <h4 className="text-teal-500 text-[10px] font-black uppercase mb-6 italic border-b border-slate-800 pb-2">DNA</h4>
                        <div className="space-y-3">
      ...
      <section className="w-[480px] bg-[#0b0c0e] p-10 flex flex-col gap-12 overflow-y-auto border-l border-slate-800">
        <div className="space-y-6 bg-black/30 p-8 rounded-2xl border border-slate-800">
            ...
            <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                   <select className="bg-[#161a1d] p-4 border border-slate-800 text-[11px] text-teal-500 font-black" value={newP.gender} onChange={(e) => setNewP({...newP, gender: e.target.value})}>
                      {GENDERS.map(g => <option key={g} value={g}>{g}</option>)}
                   </select>
                   <input className="bg-[#161a1d] p-4 border border-slate-800 text-[11px] text-slate-500" placeholder="ROLE" value={newP.role} onChange={(e) => setNewP({...newP, role: e.target.value})} />
                </div>
                <button onClick={spawnPersona} disabled={loading || !newP.name} className={`w-full py-4 text-[10px] font-black uppercase tracking-widest rounded shadow-xl ${loading || !newP.name ? 'bg-slate-900 text-slate-800 opacity-20' : 'bg-teal-500 text-black hover:bg-white'}`}>Commit_DNA</button>
            </div>
        </div>
        ...
        <div className="space-y-6 bg-black/30 p-8 rounded-2xl border border-slate-800">
            ...
            <div className="space-y-8">
                ...
                <select className="w-full bg-[#161a1d] p-4 border border-slate-800 text-[11px] text-teal-400 font-bold" value={newS.relationship} onChange={(e) => setNewS({...newS, relationship: e.target.value})}>
                    {DYNAMICS.map(d => <option key={d} value={d}>{d}</option>)}
                </select>
                <button onClick={handleCreateSeason} disabled={loading || !newS.topic || newS.host_ids.length !== 2} className={`w-full py-6 text-[11px] font-black uppercase tracking-widest rounded shadow-2xl ${loading || !newS.topic || newS.host_ids.length !== 2 ? 'bg-slate-900 text-slate-800 opacity-20' : 'bg-teal-500 text-black hover:bg-white'}`}>Establish_Signal</button>
            </div>
        </div>
      </section>
    </div>
