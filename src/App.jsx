...
const CONFIG = {
    ...
    DYNAMICS: ["UST - Professional Denial", "Strategic Tension", "Buddy Cop", "Master / Apprentice", "Bitter Rivals", "Frenemies", "Unresolved Sexual Tension"],
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0d0f11] text-slate-400">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Zap size={16} /> Telemetry_v38.0</div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416] min-w-[800px]">
                {loading && (
                    <div className="absolute inset-0 bg-black/80 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase tracking-[0.8em] animate-pulse italic">Synchronizing_Grounded_Apex...</p>
                    </div>
                )}

                <div className="flex gap-4 mb-12 border-b border-slate-800 pb-8">
                    <button onClick={() => {setActiveItem(null); setActiveBrief(null); setActiveTab('season');}} className={`px-12 py-4 text-[11px] font-black transition-all shadow-2xl ${activeTab === 'season' && !activeItem ? 'bg-teal-500 text-black shadow-teal-500/20' : 'bg-slate-800'}`}>SEASONS</button>
                    <button onClick={() => {setActiveItem(null); setActiveBrief(null); setActiveTab('persona');}} className={`px-12 py-4 text-[11px] font-black transition-all shadow-2xl ${activeTab === 'persona' && !activeItem ? 'bg-teal-500 text-black shadow-teal-500/20' : 'bg-slate-800'}`}>DNA_VAULT</button>
                </div>

                {!activeItem ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                        {listData.map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-10 rounded-2xl cursor-pointer hover:border-teal-500 transition-all flex gap-8 items-center shadow-2xl group" onClick={() => setActiveItem(item)}>
                    ...
                    <div className="animate-in slide-in-from-bottom-6 duration-500 space-y-12">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-10">
                            ...
                            <button onClick={() => {setActiveItem(null); setActiveBrief(null);}} className="bg-teal-500 text-black px-12 py-4 text-xs font-black shadow-2xl hover:bg-white transition-all">RETURN_TO_LIST</button>
                        </div>

                        {activeTab === 'season' && (
                            <div className="flex flex-col gap-12">
                                <div className="bg-teal-950/10 p-14 border border-teal-900/30 rounded-3xl shadow-2xl">
                                    <h4 className="text-teal-400 text-[12px] font-black uppercase mb-6 flex items-center gap-4 italic tracking-[0.3em]"><ScrollText size={24}/> Geopolitical_Arc_Grounded</h4>
                                    <p className="text-[20px] text-slate-300 font-sans leading-relaxed uppercase selection:bg-teal-500/30">{activeItem?.summary}</p>
                                </div>
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                                    ...
                                    <div className="bg-black/40 border border-slate-800 rounded-3xl p-12 h-[850px] overflow-y-auto shadow-2xl relative">
                                        ...
                                        )) : <div className="h-full flex items-center justify-center text-slate-800 text-sm font-black uppercase tracking-[0.4em] opacity-30 text-center italic">Initialize_Node_Handshake</div>}
                                    </div>
                                </div>
                            </div>
                        )}
