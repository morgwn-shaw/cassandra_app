...
const App = () => {
    ...
    // STABILITY LATCH: Kill indices on season change
    useEffect(() => {
        setActiveEpIdx(null);
    ...
    const activeSeason = (seasons || []).find(x => x.id === activeId) || null;
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px]">
                ...
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-20 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full h-full border border-teal-900/40 rounded-3xl p-12 flex gap-12 overflow-hidden relative shadow-2xl animate-in zoom-in duration-300" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-6 space-y-8 select-text">
                            <div className="bg-black/40 p-8 rounded-2xl border border-slate-800"><h4 className="text-teal-400 font-black mb-4 uppercase tracking-widest">DNA_Protocol</h4><p className="text-slate-300 text-sm leading-relaxed uppercase">{viewPersona?.archive?.bio || "Syncing..."}</p></div>
                            <div className="space-y-4"><h4 className="text-teal-400 font-black uppercase tracking-widest">Anecdote History</h4>
            ...
            <aside className="w-[280px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="flex items-center gap-3 text-teal-500 font-black uppercase tracking-widest border-b border-teal-900/30 pb-4"><Cpu size={16}/> Apex_v162.0</div>
                <button onClick={sync} className="w-full p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all shadow-lg"><Wifi size={14}/> Handshake</button>
                            ...
                            <div className="flex-1 bg-black border border-teal-900/30 p-10 rounded-[40px] overflow-auto shadow-2xl relative custom-scrollbar select-text cursor-text">
                                <div className="sticky top-0 bg-black/95 backdrop-blur-xl pb-6 mb-6 border-b border-teal-900/20 flex justify-between items-center z-10">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {terminalMode === 'script' && <button onClick={() => { navigator.clipboard.writeText(JSON.stringify(getMasterScript())); window.alert("JSON_COPIED"); }} className="bg-teal-600 text-black px-6 py-2 rounded-full text-[9px] font-black uppercase hover:bg-white flex items-center gap-2 transition-all"><Copy size={14}/> Copy Master</button>}
                                    </div>
                                </div>
                                            ...
                                            {getMasterScript().length > 0 ? getMasterScript().map((line, idx) => (
                                                <div key={idx} className="border-l-2 border-teal-900/40 pl-6 group">
                                                    <div className="flex gap-4 items-center mb-1">
                                                       ...
                                                       <span className="text-[7px] text-slate-700 opacity-0 group-hover:opacity-100 transition-opacity">Speed: {line.speed} | Emotion: {line.emotion}</span>
                                                    </div>
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                <div className="space-y-6">
                    <h3 className="text-teal-500 text-[11px] font-black uppercase border-b border-slate-800 pb-3 flex items-center gap-2 tracking-widest"><UserPlus size={18}/> Identity Spawn</h3>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <select className="bg-slate-900 p-5 border border-slate-800 text-[11px] text-teal-500 outline-none rounded-xl" value={nP.gender} onChange={e => setNP({...nP, gender: e.target.value})}>{CONFIG.G.map(g => <option key={g} value={g}>{g}</option>)}</select>
                    ...
                    <div className="flex flex-wrap gap-3 py-6 border-t border-slate-900">
                        {(personas || []).map(p => (
                            <div key={p.id} className="relative group cursor-pointer" onClick={() => setViewPersona(p)}>
                                <img src={p.portrait} className="w-16 h-16 rounded-2xl border-2 border-slate-800 group-hover:border-teal-500 transition-all bg-black object-cover shadow-lg" onError={e => e.target.src="https://via.placeholder.com/100?text=?"} />
                ...
                <div className="space-y-6 border-t border-slate-900 pt-8">
                    <h3 className="text-teal-500 text-[11px] font-black uppercase border-b border-slate-800 pb-3 flex items-center gap-2 tracking-widest"><Archive size={18}/> Establish Season</h3>
