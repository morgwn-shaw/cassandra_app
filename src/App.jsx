...
const SafeBrief = ({ data }) => {
    ...
    return Object.entries(data).map(([act, content], idx) => {
        // Support both flat strings and the new nested objects
        const title = typeof content === 'object' ? content.title : act.replace('_', ' ');
        const briefText = typeof content === 'object' ? content.brief : content;
        ...
        return (
            <div key={idx} className="mb-6 border-l-2 border-teal-900/30 pl-4 animate-in slide-in-from-left duration-300">
                ...
                <p className="text-sm text-slate-300 italic leading-relaxed mt-1 uppercase">"{briefText}"</p>
            </div>
        );
    });
};
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            ...
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v140.0</div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest italic animate-pulse">Synchronizing Reality Signal...</p></div>}
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="bg-black/40 p-6 border border-slate-800 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[9px] font-black uppercase mb-4 flex items-center gap-2"><FastForward size={14}/> Roadmap</h4><p className="text-[11px] text-slate-400 leading-relaxed italic">{active?.lore?.future_lore || "Syncing signal..."}</p></div>
                                </div>
                                <div className="space-y-4">{(active?.episodes || []).map((e, idx) => (
                                    <div key={idx} className="bg-[#1c1f23] p-6 border border-slate-800 rounded-2xl flex justify-between items-center group hover:border-teal-500 transition-all shadow-lg shadow-black/50">
                                        ...
                                        <button onClick={() => run('/showrunner/brief', { title: e.title, phase: e.phase, season_id: active.id }, 'POST', true).then(d => setBrief(d?.acts))} className="p-3 bg-slate-900 text-teal-500 rounded-full hover:bg-teal-500 hover:text-black transition-all active:scale-95"><PlayCircle size={20}/></button>
                                    </div>
                                ))}</div>
                            </div>
