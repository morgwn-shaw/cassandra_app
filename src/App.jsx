...
const App = () => {
  ...
  const refreshData = async () => {
    ...
    } catch (e) { setLiveStatus({ stage: 'ERROR', last_event: 'API Offline' }); }
  };

  const handleCreateSeason = async () => {
    if (!newS.topic || newS.host_ids.length === 0) return alert("Topic and Hosts Required.");
    setLoading(true);
    try {
  ...
  const deleteSeason = async (id) => {
    if (window.confirm("CONFIRM DELETION: Purge this arc from the archive?")) {
    ...
    <div className="min-h-screen bg-[#050505] text-zinc-300 font-mono flex">
      ...
      <nav className="w-80 border-r border-zinc-900 bg-black/50 p-8 flex flex-col shadow-2xl">
        <div className="text-[#00ffcc] font-black uppercase text-[10px] mb-8 flex items-center gap-2 tracking-[0.2em]">
          <div className="w-2 h-2 bg-[#00ffcc] animate-pulse" /> Cassandra_Shadow_v19
        </div>
        ...
        <div className={`mt-8 p-5 border rounded bg-black/80 transition-all ${liveStatus.stage === 'ERROR' ? 'border-red-500/50' : 'border-zinc-900'}`}>
           <p className="text-zinc-600 mb-2 flex items-center gap-2 uppercase text-[8px] font-black"><Activity size={10}/> Telemetry_Log</p>
           <div className={`font-black uppercase text-[10px] mb-2 ${liveStatus.stage === 'ERROR' ? 'text-red-500 underline' : 'text-[#00ffcc]'}`}>{liveStatus.stage}</div>
           <div className="text-zinc-500 text-[9px] leading-relaxed break-words font-bold italic">
             {liveStatus.last_event}
           </div>
           {liveStatus.stage === 'ERROR' && (
             <div className="mt-4 flex items-center gap-2 text-red-400 text-[8px] font-black animate-pulse">
               <AlertCircle size={10}/> [ACTION_REQUIRED]
             </div>
           )}
        </div>
      ...
      <main className="flex-1 p-16 overflow-y-auto">
        ...
        {activeTab === 'season' && (
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
             {seasons.map((s, i) => (
               <div key={i} className="border border-zinc-900 bg-zinc-950 p-8 rounded relative hover:border-[#00ffcc]/30 transition-all">
                 <button onClick={() => deleteSeason(s.id)} className="absolute top-4 right-4 text-zinc-700 hover:text-red-500 transition-all z-10">
                   <Trash2 size={18}/>
                 </button>
                 <div onClick={() => { setActiveSeason(s); setActiveTab('vault'); }} className="cursor-pointer">
                    <h4 className="text-sm font-black uppercase text-white mb-2 pr-8">{s.title}</h4>
                    <p className="text-[9px] text-zinc-600 mb-6 italic">{s.relationship}</p>
                    <div className="w-full py-4 bg-zinc-900 text-[#00ffcc] text-[9px] font-black uppercase flex items-center justify-center gap-3 border border-zinc-800 hover:bg-[#00ffcc] hover:text-black transition-all">
           ...
           <div className="animate-in fade-in duration-500">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div className="border border-zinc-900 p-12 rounded bg-zinc-950/50">
                  <h3 className="text-zinc-500 text-xs font-black uppercase mb-8 border-b border-zinc-900 pb-4 italic">Metadata</h3>
                  <div className="text-[10px] space-y-4">
                    <p><span className="text-zinc-600 uppercase">Audit_ID:</span> {activeSeason.id}</p>
                    <p><span className="text-zinc-600 uppercase">Model:</span> {activeSeason.model_version}</p>
                    <p><span className="text-zinc-600 uppercase">Hosts:</span> {activeSeason.hosts?.map(h => h.name).join(' // ')}</p>
                  </div>
                </div>
                <div className="border border-[#ff4444]/20 p-12 rounded bg-black/80 shadow-2xl">
                  <h3 className="text-[#ff4444] text-xs font-black uppercase mb-8 border-b border-[#ff4444]/10 pb-4 italic tracking-widest">Cynical_Lore_Output</h3>
        ...
        <div className="fixed inset-0 bg-black/95 z-[100] flex items-center justify-center p-6 backdrop-blur-md">
          <div className="bg-zinc-950 border border-zinc-800 p-12 rounded-2xl max-w-xl w-full">
            ...
            <div className="space-y-10">
              ...
              <button onClick={handleCreateSeason} disabled={loading} className={`w-full py-6 font-black uppercase transition-all ${loading ? 'bg-zinc-900 text-[#00ffcc] animate-pulse' : 'bg-[#00ffcc] text-black hover:bg-white shadow-lg'}`}>
