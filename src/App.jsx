...
const SafeBrief = ({ data }) => {
    ...
    return Object.entries(data).map(([act, text], idx) => (
        <div key={idx} className="mb-6 border-l-2 border-teal-900/30 pl-4 animate-in slide-in-from-left duration-300">
            ...
            <p className="text-sm text-slate-400 italic leading-relaxed uppercase">"{text}"</p>
        </div>
    ));
};
...
const App = () => {
    ...
    // PURGE TRIGGER: Clears volatile data when context changes to prevent white-screens
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v134.0</div>
                ...
                <div className="space-y-2">
                    ...
                    <button onClick={() => {setLoad(false); sync();}} className="w-full p-2 text-[9px] text-teal-900 font-black uppercase hover:text-teal-400 flex items-center justify-center gap-2"><RefreshCw size={12}/> Clear Signal</button>
                </div>
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> Forensic_Intelligence</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active?.summary || "Analyzing node cluster..."}</p></div>
