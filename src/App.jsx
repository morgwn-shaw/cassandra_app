...
const App = () => {
    ...
    const ping = async () => {
        ...
        } catch (e) { window.alert("Failed: " + e.message); } finally { setLoad(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden">
            <aside className="w-[280px] border-r border-slate-800 bg-black/60 p-6 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-2 text-teal-500 font-black text-xs uppercase"><Cpu size={14}/> Apex_v58.0</div>
                    <div className="text-[9px] text-teal-900 uppercase font-black mt-1 tracking-widest">{eng}</div>
                </div>
                <button onClick={ping} className="p-3 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase flex items-center gap-2 hover:bg-teal-500 hover:text-black transition-all rounded shadow-lg"><Wifi size={12}/> Handshake</button>
                <div className="mt-auto space-y-4">
                    ...
                    <button onClick={() => run('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition-all">Reset</button>
                </div>
            </aside>

            <main className="flex-1 flex flex-col p-10 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/90 z-[100] flex flex-col items-center justify-center backdrop-blur-md"><Loader2 className="animate-spin text-teal-500 mb-2"/><p className="text-teal-500 text-[10px] font-black">EXTRACTING...</p></div>}
                    ...
                    <div className="grid grid-cols-2 gap-6">
                        {(tab === 'season' ? seasons : personas).map((i, k) => (
                            <div key={k} className="bg-[#1c1f23] border border-slate-800 p-6 rounded-2xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95" onClick={() => setActive(i)}>
                                {view === 'god' && <button onClick={async (e) => { e.stopPropagation(); await fetch("[https://shadow-cassandrafiles.pythonanywhere.com/api/v2/delete/](https://shadow-cassandrafiles.pythonanywhere.com/api/v2/delete/)" + tab + "/" + i.id, {method:'DELETE'}); sync(); }} className="absolute top-4 right-4 text-red-900 hover:text-red-500"><Trash2 size={14}/></button>}
                                <img src={i.portrait} className="w-16 h-16 rounded-xl grayscale group-hover:grayscale-0 transition-all" alt="D"/>
                                <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-lg italic truncate leading-none">{i.title || i.name}</h4><p className="text-[9px] text-teal-500 font-black opacity-60 uppercase mt-2">{i.rel || i.archive?.mbti}</p></div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="space-y-10 animate-in slide-in-from-bottom-4">
                        ...
                        {tab === 'season' && (
                            <div className="space-y-10">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl"><h4 className="text-teal-400 text-[10px] font-black uppercase mb-4 italic tracking-widest">Grounded_Arc</h4><p className="text-lg text-slate-300 uppercase">{active.summary}</p></div>
            ...
            <aside className="w-[380px] bg-[#0b0c0e] p-10 flex flex-col gap-12 border-l border-slate-800 overflow-y-auto shrink-0">
                ...
                <div className="space-y-6 border-t border-slate-900 pt-10"><h3 className="text-teal-400 text-[10px] font-black uppercase flex items-center gap-2 border-b border-slate-900 pb-3 tracking-widest"><Database size={16}/> Establish</h3><input className="w-full bg-[#1c1f23] p-4 border border-slate-800 text-xs text-white font-bold outline-none focus:border-teal-500 uppercase" placeholder="TOPIC" value={nS.topic} onChange={(e) => setNS({...nS, topic: e.target.value})} /><select className="w-full bg-[#1c1f23] p-4 border border-slate-800 text-[10px] text-teal-400 font-bold outline-none" value={nS.relationship} onChange={(e) => setNS({...nS, relationship: e.target.value})}>{C.D.map(d => <option key={d}>{d}</option>)}</select><div className="grid grid-cols-2 gap-2 max-h-[200px] overflow-y-auto p-4 border border-slate-800 rounded-xl bg-black/40 custom-scrollbar">{(personas || []).map(p => (<button key={p.id} onClick={() => { const ids = nS.host_ids.includes(p.id) ? nS.host_ids.filter(id => id !== p.id) : [...nS.host_ids, p.id]; setNS({...nS, host_ids: ids.slice(0, 2)}); }} className={`p-3 text-[9px] font-black border uppercase rounded-lg truncate ${nS.host_ids.includes(p.id) ? 'border-teal-500 bg-teal-500/10 text-teal-400' : 'border-slate-800 text-slate-600'}`}>{p.name}</button>))}</div><button onClick={() => run('/season/reconcile', nS)} disabled={load || nS.host_ids.length !== 2} className="w-full py-5 bg-teal-500 text-black text-[10px] font-black uppercase shadow-2xl hover:bg-white active:scale-95 transition-all">Establish_Signal</button></div>
            </aside>
        </div>
    );
};
