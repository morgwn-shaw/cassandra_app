...
import { Zap, Trash2, Database, Quote, Loader2, UserPlus, History, FastForward, Wifi, Cpu, ThumbsUp, ThumbsDown, Dice5, ScrollText, PlayCircle, Globe, Calendar, RefreshCw, Music } from 'lucide-react';
...
const App = () => {
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            if (!r.ok) throw new Error(d.error || "Handshake Crash");
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                ...
                <div className="space-y-2">
                    <button onClick={ping} className="w-full p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 rounded transition-all shadow-lg"><Wifi size={14}/> Handshake</button>
                    <button onClick={() => setLoad(false)} className="w-full p-2 text-[9px] text-teal-900 font-black uppercase hover:text-teal-400 flex items-center justify-center gap-2"><RefreshCw size={12}/> Clear Signal Blocker</button>
                </div>
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-900"><button onClick={() => run('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black border border-red-900/30 hover:bg-red-600 transition-all uppercase italic">Reset Vault</button></div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                ...
                <div className="flex gap-4 mb-10 border-b border-slate-800 pb-8">
                    <button onClick={() => {setActive(null); setTab('season'); setBrief(null);}} className={`px-12 py-4 text-[11px] font-black transition-all ${tab === 'season' && !active ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>SEASONS</button>
                    <button onClick={() => {setActive(null); setTab('persona'); setBrief(null);}} className={`px-12 py-4 text-[11px] font-black transition-all ${tab === 'persona' && !active ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>PERSONAS</button>
                </div>
                ...
                {!active ? (<div className="grid grid-cols-2 gap-8 animate-in fade-in">
                    {(tab === 'season' ? seasons : personas).map((i, k) => (
                        <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all shadow-inner shadow-black/80" onClick={() => setActive(i)}>
                            ...
                            <img src={i?.portrait} className="w-20 h-20 rounded-2xl grayscale transition-all shadow-xl" alt="DNA"/>
            ...
            <aside className="w-[420px] bg-[#0b0c0e] p-12 flex flex-col gap-14 border-l border-slate-800 overflow-y-auto shrink-0 shadow-2xl shadow-black/80">
                <div className="space-y-8"><h3 className="text-teal-400 text-[11px] font-black uppercase border-b border-slate-900 pb-4 tracking-widest"><UserPlus size={20}/> Identity_Spawn</h3>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <input className="bg-[#1c1f23] p-5 border border-slate-800 text-[11px] text-slate-500 uppercase shadow-inner outline-none focus:border-teal-500" placeholder="ROLE" value={nP.role} onChange={(e) => setNP({...nP, role: e.target.value})} />
                    </div>
                    <div className="relative">
                        <input className="w-full bg-[#1c1f23] p-5 border border-slate-800 text-[10px] text-slate-500 outline-none uppercase pr-14 shadow-inner focus:border-teal-500" placeholder="CORE_TRAUMA" value={nP.trauma} onChange={(e) => setNP({...nP, trauma: e.target.value})} />
