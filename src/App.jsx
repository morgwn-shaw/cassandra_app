        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px] select-none">
            ...
            {viewPersona && (
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-12 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative" onClick={e => e.stopPropagation()}>
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0">
                <div className="flex items-center gap-3 text-teal-500 font-black uppercase tracking-widest border-b border-teal-900/30 pb-4 italic"><Cpu size={18}/> Apex_v176</div>
                        ...
                        {viewLore ? (
                            ...
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-12 flex flex-col gap-8 overflow-hidden relative shadow-inner">
                                ...
                                <div className="space-y-3 border-b border-slate-800 pb-8">
                                    ...
                                    <p className="text-teal-500 uppercase font-black text-[11px] tracking-[0.3em] italic text-center text-center font-black">PRODUCING...</p>
                                </div>
                                <div className="flex-1 overflow-y-auto custom-scrollbar pr-6">
                                    {activeSeason.episodes[activeEp].full_script ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full gap-8">
                                            ...
                                            <button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl">Launch Producer</button>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">
                                {activeSeason.episodes.map((e, idx) => (
                                    <div key={idx} onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500/50 cursor-pointer transition-all shadow-xl group h-fit">
                                        <div className="text-[10px] text-teal-800 font-black uppercase mb-3 tracking-widest italic text-center">NODE_{idx + 1}</div>
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-12 shadow-2xl custom-scrollbar">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <button onClick={createSeason} disabled={nS.host_ids.length !== 2 || !nS.topic} className="w-full py-6 bg-teal-500 text-black font-black uppercase text-[12px] rounded-[1.5rem] shadow-2xl shadow-teal-500/10">Establish Season</button>
                </div>
            </aside>
        </div>
    );
