...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V182_STABLE", type: "system" }]);

    const [nP, setNP] = useState({ name: '', role: 'Host', gender: 'Female', description: DESC_SAMPLES[Math.floor(Math.random()*DESC_SAMPLES.length)] });
    const [nS, setNS] = useState({ topic: '', relationship: 'Frenemies', host_ids: [], episodes_count: 8, target_runtime: 15 });
    ...
    const createSeason = async () => {
        setLoad(true); setStatus("Mapping Season Arc...");
        try {
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <aside className="w-[240px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0">
                ...
                <div ref={logRef} className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar text-[9px] uppercase bg-black/20">
                    {logs.map((log, i) => (
                        <div key={i} className={`p-2 rounded border ${log.type === 'error' ? 'bg-red-950/20 border-red-900/40 text-red-500' : 'bg-slate-900/40 border-slate-800 text-slate-500'}`}>
                            {log.m.substring(0, 80)}
                        </div>
                    ))}
                </div>
            </aside>
                                ...
                                <div className="flex-1 overflow-y-auto custom-scrollbar pr-6 space-y-10">
                                    {activeSeason?.episodes?.[activeEp]?.full_script ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full gap-8">
                                            ...
                                            <button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl">Produce 25m Script</button>
                                        </div>
                                    )}
                                </div>
                            </div>
            ...
            <aside className="w-[340px] bg-black/60 p-8 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                ...
                <div className="space-y-6">
                    ...
                    <div className="relative group">
                        <textarea className="w-full bg-slate-900/50 p-4 border border-slate-800 text-slate-300 rounded-xl outline-none focus:border-teal-500 uppercase text-[10px] h-32 custom-scrollbar leading-relaxed" placeholder="DESCRIPTION" value={nP.description} onChange={e => setNP({...nP, description: e.target.value})} />
                        <Sparkles size={14} className="absolute bottom-4 right-4 text-teal-900 group-hover:text-teal-500 transition-colors pointer-events-none" />
                    </div>
                    ...
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="w-[300px] shrink-0 space-y-6">
                            ...
                            <div className="p-4 bg-teal-950/20 border border-teal-900/30 rounded-xl">
                                ...
                                <div className="text-white font-black text-[12px]">{viewPersona?.dna?.mbti} Type</div>
                            </div>
                            <div className="bg-black/40 p-4 border border-slate-800 rounded-xl">
                                <h4 className="text-teal-500 font-black uppercase italic text-[9px] mb-2 flex items-center gap-2"><Sparkles size={10}/> Intro Statement</h4>
                                <p className="text-slate-300 text-[11px] leading-relaxed select-text italic">"{viewPersona?.dna?.intro}"</p>
                            </div>
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-8">
                            <div className="p-8 bg-black/40 border border-slate-800 rounded-2xl">
                                <h4 className="text-teal-500 font-black mb-4 uppercase italic tracking-widest text-[10px] flex items-center gap-2"><UserCircle size={14}/> Behavioral DNA</h4>
                                <p className="text-slate-200 uppercase leading-relaxed text-[12px] select-text">{viewPersona?.dna?.personality_blurb}</p>
                            </div>
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-4 border-l border-teal-900/30 pl-6"><h4 className="text-teal-500 font-black uppercase italic text-[10px]">20 Cultural Likes</h4><div className="flex flex-wrap gap-2">{(viewPersona?.dna?.likes || []).map((l, i) => <span key={i} className="px-2 py-1 bg-teal-500/5 border border-teal-500/10 text-teal-400 text-[9px] rounded uppercase">{l}</span>)}</div></div>
                            ...
                            <div className="space-y-4 pt-6 border-t border-slate-800">
                                <h4 className="text-teal-500 font-black uppercase italic text-[10px] tracking-widest">25 Core Memories</h4>
                                <div className="grid grid-cols-1 gap-2">
                                    {(viewPersona?.dna?.core_memories || []).map((m, i) => (
                                        <div key={i} className="p-3 bg-white/5 rounded-xl text-[10px] italic border border-white/5 uppercase select-text leading-relaxed opacity-70 hover:opacity-100 transition-opacity">[{i+1}] {m}</div>
                                    ))}
                                </div>
                            </div>
                        </div>
