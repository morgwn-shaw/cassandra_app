...
const App = () => {
    ...
    const [terminalMode, setTerminalMode] = useState('brief'); // brief, script
    ...
    // STABILITY LOCK: Reset episode index whenever season changes to prevent out-of-bounds crash
    useEffect(() => {
        setActiveEpIdx(null);
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px]">
            {viewPersona && (
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-20 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full h-full border border-teal-900/40 rounded-3xl p-12 flex gap-12 overflow-hidden relative" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-6 space-y-8 text-selectable">
            ...
            <aside className="w-[280px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="flex items-center gap-3 text-teal-500 font-black uppercase tracking-widest border-b border-teal-900/30 pb-4"><Cpu size={16}/> Apex_v160.0</div>
                <button onClick={sync} className="w-full p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all shadow-lg"><Wifi size={14}/> Sync Stream</button>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-10 bg-[#0d0f11] relative overflow-hidden">
                ...
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0">
                    ...
                    {activeId && <div className="flex-1 text-teal-500 font-black uppercase flex items-center gap-2 italic truncate tracking-widest underline decoration-teal-900 underline-offset-8">Production: {activeSeason?.title}</div>}
                </div>
                            ...
                            <div className="flex-1 bg-black border border-teal-900/30 p-10 rounded-[40px] overflow-auto shadow-2xl relative custom-scrollbar select-text cursor-text">
                                <div className="sticky top-0 bg-black/95 backdrop-blur-xl pb-6 mb-6 border-b border-teal-900/20 flex justify-between items-center z-10">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        {terminalMode === 'script' && <button onClick={() => { navigator.clipboard.writeText(JSON.stringify(getMasterScript())); window.alert("JSON_COPIED"); }} className="bg-teal-600 text-black px-6 py-2 rounded-full text-[9px] font-black hover:bg-white flex items-center gap-2 transition-all"><Copy size={14}/> Copy Master</button>}
                                    </div>
                                </div>
                                
                                <div className="selection:bg-teal-500 selection:text-black">
                                    {terminalMode === 'brief' && <pre className="text-teal-400 text-[13px] leading-relaxed whitespace-pre-wrap italic uppercase p-4">{activeEp.saved_brief || "Select episode and generate brief."}</pre>}
                                    {terminalMode === 'script' && (
                                        <div className="space-y-6">
                                            ...
                                            )) : <div className="h-40 flex items-center justify-center text-slate-700 italic">Generate Acts 1-6 in the Brief tab to stitch the script.</div>}
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="flex flex-wrap gap-3 py-6 border-t border-slate-900">
                        {personas.map(p => (
                            <div key={p.id} className="relative group cursor-pointer" onClick={() => setViewPersona(p)}>
                                ...
                                <button onClick={(e) => { e.stopPropagation(); run(`/delete/persona/${p.id}`, null, 'DELETE'); }} className="absolute -top-2 -right-2 bg-red-600 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-all border-2 border-black"><Trash2 size={10}/></button>
                            </div>
                        ))}
                    </div>
                </div>
