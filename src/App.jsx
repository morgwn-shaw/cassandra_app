...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, Terminal, Mic2, Headphones, Sparkles, UserCircle, MessageSquare, Copy, BarChart3, ClipboardCheck, Info, Volume2, Wand2 } from 'lucide-react';
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V202_MASTER_RESTORED", type: "system" }]);
    ...
    const deleteEpisode = async (idx) => {
        ...
        setLoad(true); setStatus("Pruning branch...");
    ...
    const stitchAudio = async () => {
        ...
        try {
            ...
            const data = await res.json(); setMasterAudio(data.master_url);
        } finally { setLoad(false); setStatus(""); }
    };
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0">
                ...
                <div ref={logRef} className="flex-1 overflow-y-auto p-4 space-y-2 opacity-60 custom-scrollbar text-[9px] uppercase">{logs.map((l, i) => <div key={i}>[{l.t}] {l.m}</div>)}</div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); setActiveEp(null); setAudioManifest(null); setMasterAudio(null); }} className={`px-8 py-3 font-black uppercase rounded-lg ${!activeId ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-800'}`}>Library</button>
                    {activeId && <><div className="flex-1 text-teal-500 font-black uppercase italic truncate text-lg px-4 truncate"><ChevronRight className="inline mr-2"/>{activeSeason?.title}</div><button onClick={() => setViewLore(!viewLore)} className={`p-4 rounded-xl border flex items-center gap-2 uppercase font-black transition-all ${viewLore ? 'bg-teal-500 text-black border-teal-500' : 'border-slate-800 text-slate-500'}`}><History size={16}/> Lore</button></>}
                </div>
                    ...
                    <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col overflow-hidden relative shadow-2xl">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">
                            {audioManifest ? (
                                ...
                                <>
                                    <textarea readOnly className="w-full h-[40vh] bg-slate-950/80 p-8 rounded-3xl border border-teal-900/20 text-teal-300 font-mono text-[9px] resize-none outline-none select-text custom-scrollbar" value={JSON.stringify(activeSeason.episodes[activeEp].full_script_blocks, null, 4)} />
                                    <div className="bg-teal-950/10 p-8 border border-teal-900/30 rounded-3xl shadow-xl">
                                        <h4 className="text-teal-500 font-black uppercase italic flex items-center gap-2 mb-4"><ClipboardCheck size={20}/> Post-Production Assessment</h4>
