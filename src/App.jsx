...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, Terminal, Mic2, Headphones, Sparkles, UserCircle, MessageSquare } from 'lucide-react';

const BASE_URL = "https://shadow-cassandrafiles.pythonanywhere.com/api/v2";
const DESC_SAMPLES = [
    "A jaded crypto-archaeologist who treats data like physical artifacts. Speaks in staccato, high curiosity.",
    "A former socialite turned radical environmentalist. Elegant cadence, deeply inquisitive, high risk taker.",
    "A hardware hacker who sees patterns in white noise. Soft-spoken, monotone, intense curiosity.",
    "A retired cleaner for megacorps. Gravelly voice, cynical, risk-averse."
];
const CONFIG = {
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V190_TTS_UP", type: "system" }]);
    ...
    const createSeason = async () => {
        ...
        setLoad(true);
        try {
            setStatus("Step 1: Init Skeleton..."); addLog(`INIT: ${nS.topic}`, "system");
            const r1 = await fetch(`${BASE_URL}/season/init`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(nS) });
            const s = await r1.json();
            
            setStatus("Step 2: Narrative Research..."); addLog("RESEARCH_START...", "system");
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    ...
                    {activeId && <div className="flex-1 text-teal-500 font-black uppercase italic truncate text-lg"><ChevronRight className="inline mx-2"/>{activeSeason?.title || "LOAD"}</div>}
                </div>

                {!activeId ? (
                    <div className="grid grid-cols-2 gap-8 overflow-y-auto pr-4 custom-scrollbar">
                        {seasons?.map(s => (
                            <div key={s.id} onClick={() => setActiveId(s.id)} className="bg-[#1c1f23] p-10 border border-slate-800 rounded-[2rem] cursor-pointer hover:border-teal-500 transition-all relative group h-fit">
                    ...
                    <div className="flex flex-col h-full gap-8 overflow-hidden">
                        {activeEp !== null ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-12 overflow-y-auto custom-scrollbar relative">
                                <button onClick={() => setActiveEp(null)} className="absolute top-8 right-12 text-slate-500 hover:text-white uppercase font-black italic">[ Close ]</button>
                                {activeSeason?.episodes?.[activeEp]?.full_script_blocks ? (
                                    <div className="space-y-4">
                                        {activeSeason.episodes[activeEp].full_script_blocks.map((block, bIdx) => (
                                            <div key={bIdx} className="bg-slate-900/60 border border-slate-800 p-6 rounded-2xl flex gap-6 items-start">
                                                <div className="w-24 shrink-0">
                                                    <div className="text-teal-500 font-black uppercase text-[10px]">{block.name}</div>
                                                    <div className="text-slate-600 text-[8px] uppercase mt-1">{block.style}</div>
                                                </div>
                                                <div className="flex-1 text-slate-200 text-[12px] leading-relaxed select-text italic">{block.text}</div>
                                                <div className="text-teal-900 font-black text-[8px]">{block.speed}x</div>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <div className="flex flex-col items-center justify-center h-[50vh] gap-8">
                                        ...
                                        <button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl">Launch Performance Engine</button>
                                    </div>
                                )}
                            </div>
                        ) : (
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">
                                {activeSeason?.episodes?.map((e, idx) => (
                                    <div key={idx} onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500 cursor-pointer text-center h-fit">
                                        ...
                                        {e.full_script_blocks && <div className="mt-4 text-teal-500 text-[10px] font-black uppercase italic flex items-center justify-center gap-2"><Zap size={12}/> production_locked</div>}
                                    </div>
                                ))}
                            </div>
                        )}
            ...
            <aside className="w-[340px] bg-black/60 p-8 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <select className="bg-slate-900/50 p-4 border border-slate-800 text-teal-500 rounded-2xl outline-none uppercase font-black" value={nP.role} onChange={e => setNP({...nP, role: e.target.value})}>
                            <option value="Host">Host</option><option value="Analyst">Analyst</option><option value="Skeptic">Skeptic</option>
                        </select>
                    </div>
                    <button onClick={createPersona} disabled={!nP.name || load} className="w-full py-5 bg-teal-500 text-black font-black uppercase rounded-[1.5rem] shadow-2xl">Commit Subject</button>
                    <div className="flex flex-wrap gap-4 pt-6 border-t border-slate-900">
                        {personas?.map(p => (
                            <div key={p.id} className="relative group">
                                <img src={p.portrait} onClick={() => setViewPersona(p)} className="w-14 h-14 rounded-xl border-2 border-slate-800 hover:border-teal-500 cursor-pointer bg-black" alt="p" />
                                <button onClick={(e) => { e.stopPropagation(); deleteItem('persona', p.id); }} className="absolute -top-2 -right-2 bg-red-600 text-white p-1 rounded-full opacity-0 group-hover:opacity-100 transition-all"><Trash2 size={10}/></button>
                            </div>
                        ))}
                    </div>
                </div>
                    ...
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-4 border-l border-teal-900/30 pl-6 text-[9px] uppercase"><div className="font-black text-teal-500 mb-2 italic text-[10px]">Cultural Likes</div>{viewPersona?.dna?.likes?.map((l, i) => <span key={i} className="mr-2 opacity-70">[{l}]</span>)}</div>
