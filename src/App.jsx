    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px] select-none">
            ...
            {viewPersona && (
                <div className="fixed inset-0 z-[200] bg-black/95 flex items-center justify-center p-12 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        <button onClick={() => setViewPersona(null)} className="absolute top-6 right-6 text-slate-500 hover:text-white"><X size={28}/></button>
                        ...
                        <div className="w-[280px] shrink-0 space-y-6">
                            ...
                            <div className="space-y-4 pt-4 border-t border-slate-800 h-[320px] overflow-y-auto custom-scrollbar pr-2">
                                <div className="space-y-2">
                                    <h4 className="text-teal-500 font-black text-[9px] uppercase tracking-widest">Preferences_Like</h4>
                                    <div className="flex flex-wrap gap-1">
                                        {(viewPersona?.archive?.likes || []).map((l, i) => (
                                            <span key={i} className="px-2 py-1 bg-teal-500/5 border border-teal-500/20 text-teal-400 text-[8px] rounded uppercase">{l}</span>
                                        ))}
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    <h4 className="text-red-500 font-black text-[9px] uppercase tracking-widest">Aversions_Dislike</h4>
                                    <div className="flex flex-wrap gap-1">
                                        {(viewPersona?.archive?.dislikes || []).map((d, i) => (
                                            <span key={i} className="px-2 py-1 bg-red-500/5 border border-red-500/20 text-red-400 text-[8px] rounded uppercase">{d}</span>
                                        ))}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex-1 overflow-y-auto space-y-8 select-text custom-scrollbar pr-4">
                            <div className="bg-black/40 p-6 rounded-2xl border border-slate-800">
                                <h4 className="text-teal-500 font-black mb-2 uppercase flex items-center gap-2"><BookOpen size={12}/> Behavioral_Bio</h4>
                                <p className="text-slate-300 text-[11px] leading-relaxed uppercase">{viewPersona?.archive?.bio}</p>
                            </div>
                            <div className="space-y-3">
                                <h4 className="text-teal-500 font-black uppercase flex items-center gap-2"><History size={12}/> Historical_Anecdotes</h4>
                                {(viewPersona?.archive?.anecdotes || []).map((a, i) => (
                                    <p key={i} className="p-4 bg-white/5 rounded-xl text-[10px] italic uppercase border border-white/5">"{a}"</p>
                                ))}
                            </div>
                        </div>
                    </div>
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="flex items-center gap-3 text-teal-500 font-black uppercase tracking-widest border-b border-teal-900/30 pb-4 italic"><Cpu size={16}/> Apex_v168.2</div>
                <button onClick={sync} className="w-full p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all flex items-center justify-center gap-2">
            ...
            <main className="flex-1 flex flex-col p-10 bg-[#0d0f11] relative overflow-hidden">
                ...
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); }} className={`px-8 py-3 font-black text-[10px] uppercase transition-all rounded-lg ${!activeId ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>Library</button>
                    {activeId && (
                        <>
                            <ChevronRight size={16} className="text-slate-700"/>
                            <div className="flex-1 text-teal-500 font-black uppercase italic truncate tracking-widest">{activeSeason?.title}</div>
                    ...
                    <div className="grid grid-cols-2 gap-6 overflow-y-auto pr-4 custom-scrollbar">
                        {seasons.map(s => (
                            <div key={s.id} onClick={() => setActiveId(s.id)} className="bg-[#1c1f23] p-8 border border-slate-800 rounded-3xl cursor-pointer hover:border-teal-500 transition-all relative h-fit shadow-xl group">
                                ...
                                <div className="mt-2 text-teal-600 font-black uppercase text-[10px] italic">{s.rel} | {s.episodes?.length || 0} Nodes</div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="flex flex-col h-full gap-6 overflow-hidden">
                        {viewLore ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-3xl p-8 overflow-y-auto custom-scrollbar">
                                <h3 className="text-teal-500 font-black uppercase mb-6 flex items-center gap-2 border-b border-teal-900/20 pb-4 italic underline underline-offset-8 decoration-teal-500/30 text-lg tracking-widest text-center">Shared History Log</h3>
                                <div className="space-y-4">
                                    {(activeSeason?.shared_history || []).map((lore, i) => (
                                        <div key={i} className="p-6 bg-slate-900/50 border border-slate-800 rounded-2xl text-slate-300 uppercase text-[11px] leading-relaxed">
                            ...
                            <div className="grid grid-cols-3 gap-4 overflow-y-auto pr-4 custom-scrollbar">
                                {activeSeason?.episodes?.map((e, idx) => (
                                    <div key={idx} className="p-6 border border-slate-800 bg-slate-900/30 text-slate-400 rounded-2xl hover:border-teal-500/50 transition-all shadow-xl group">
                                        <div className="text-[9px] text-teal-700 font-black uppercase mb-2">ACT_NODE_{e.node}</div>
                                        <h5 className="text-white font-black uppercase italic text-sm mb-3">{e.title}</h5>
                                        <p className="text-[10px] leading-relaxed uppercase line-clamp-3 text-slate-500">{e.summary}</p>
                                    </div>
                                ))}
                            </div>
                        )}
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                <div className="space-y-6">
                    ...
                    <button onClick={() => run('/persona/create', nP)} disabled={!nP.name || load} className="w-full py-5 bg-teal-500 text-black font-black uppercase text-[11px] rounded-2xl hover:bg-white transition-all shadow-2xl shadow-teal-500/20 disabled:opacity-30">Commit DNA</button>
                    
                    <div className="flex flex-wrap gap-3 pt-6 border-t border-slate-900">
                        {personas.map(p => (
                            <div key={p.id} className="relative group">
                                <img src={p.portrait} onClick={() => setViewPersona(p)} className="w-14 h-14 rounded-2xl border-2 border-slate-800 hover:border-teal-500 cursor-pointer bg-black shadow-xl" alt="p" />
                ...
                <div className="space-y-6 border-t border-slate-800 pt-8">
                    ...
                    <div className="space-y-5 p-6 bg-black/20 rounded-2xl border border-slate-800">
                        <div className="space-y-2">
                            <div className="flex justify-between text-[9px] font-black uppercase italic text-slate-500">
                                <span>Nodes (Episodes): {nS.episodes_count}</span>
                            </div>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between text-[9px] font-black uppercase italic text-slate-500">
                                <span>Target Runtime: {nS.target_runtime}m</span>
                            </div>
                    ...
                    <div className="grid grid-cols-2 gap-2 max-h-32 overflow-y-auto p-2 bg-black/20 rounded-xl border border-slate-800 custom-scrollbar">
                        {personas.map(p => (
                            ...
                            }} className={`p-3 text-[8px] font-black border truncate uppercase rounded-xl transition-all ${nS.host_ids.includes(p.id) ? 'border-teal-500 text-teal-400 bg-teal-500/10' : 'border-slate-800 text-slate-500'}`}>{p.name}</button>
                        ))}
                    </div>
                    <button onClick={() => run('/season/reconcile', nS)} disabled={nS.host_ids.length !== 2 || !nS.topic || load} className="w-full py-5 bg-teal-500 text-black font-black uppercase text-[11px] rounded-2xl hover:bg-white transition-all shadow-2xl">Establish Season</button>
                </div>
            </aside>
        </div>
    );
