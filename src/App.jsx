        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0d0f11] text-slate-400">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Zap size={16} /> Telemetry_v44.5</div>
                ...
                <div className="mt-auto space-y-4">
                    ...
                    <button onClick={() => runAction('/purge', {})} className="w-full p-4 bg-red-950/20 border border-red-900/30 text-red-500 text-[10px] font-black uppercase hover:bg-red-600 hover:text-white transition-all">
                        <ShieldAlert size={16}/> Nuclear_Reset
                    </button>
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {loading && (
                    <div className="absolute inset-0 bg-black/80 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase tracking-[0.8em] animate-pulse">Establishing_Grounded_Link...</p>
                    </div>
                )}

                <div className="flex gap-4 mb-12 border-b border-slate-800 pb-8">
                    <button onClick={() => {setActiveItem(null); setActiveTab('season');}} className={`px-12 py-4 text-[11px] font-black transition-all shadow-2xl ${activeTab === 'season' && !activeItem ? 'bg-teal-500 text-black' : 'bg-slate-800'}`}>SEASONS</button>
                    <button onClick={() => {setActiveItem(null); setActiveTab('persona');}} className={`px-12 py-4 text-[11px] font-black transition-all shadow-2xl ${activeTab === 'persona' && !activeItem ? 'bg-teal-500 text-black' : 'bg-slate-800'}`}>PERSONAS</button>
                </div>
                    ...
                    <div className="animate-in slide-in-from-bottom-6 space-y-12">
                            ...
                            <div className="flex flex-col gap-12">
                                ...
                                {viewMode === 'god' && (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                        <div className="bg-[#1c1f23] p-10 border border-slate-800 rounded-2xl">
                                            <h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><History size={16}/> Shared_Lore_Vault</h4>
                                            <div className="space-y-3 h-[300px] overflow-y-auto custom-scrollbar">
                                                {(activeItem?.lore?.shared_anecdotes || []).map((a, i) => <p key={i} className="text-[11px] text-slate-500 italic uppercase border-l border-slate-800 pl-4 py-1">"{a}"</p>)}
                                            </div>
                                        </div>
                                        <div className="bg-[#1c1f23] p-10 border border-slate-800 rounded-2xl">
                                            <h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><FastForward size={16}/> Future_Relationship_Roadmap</h4>
                                            <p className="text-[14px] text-slate-400 uppercase leading-relaxed font-sans">{activeItem?.lore?.future_lore}</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 border-t border-slate-900 pt-12">
                                    <div className="space-y-6">
                                        <h4 className="text-teal-500 text-[11px] font-black uppercase italic border-b border-slate-800 pb-4">Investigation_Nodes</h4>
                                        {(activeItem?.episodes || []).map((ep, idx) => (
                                            <div key={idx} className="p-8 bg-[#1c1f23] border border-slate-800 group hover:border-teal-500 transition-all flex flex-col gap-4 shadow-xl rounded-xl">
                                                <div className="flex justify-between items-center">
                                                    ...
                                                    <button onClick={() => getBrief(activeItem.id, idx)} className="px-6 py-2 bg-slate-800 text-teal-500 text-[10px] font-black uppercase border border-slate-700 hover:bg-teal-500 hover:text-black">Ground_Brief</button>
                                                </div>
                        ...
                        {activeTab === 'persona' && (
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-in slide-in-from-bottom-6">
                                <div className="bg-black/40 p-12 border border-slate-800 rounded-3xl shadow-2xl h-[650px] overflow-y-auto custom-scrollbar">
            ...
            {/* COLUMN 3: COMMAND (RIGHT PANEL RESTORED) */}
                ...
                <div className="space-y-8 border-t border-slate-900 pt-14">
                    ...
                    <div className="grid grid-cols-2 gap-3 max-h-[220px] overflow-y-auto p-4 border border-slate-800 rounded-xl bg-black/40 custom-scrollbar shadow-inner">
                        {personas.map(p => (
                            ...
                            }} className={`p-4 text-[10px] font-black border uppercase rounded-lg truncate transition-all ${newS.host_ids.includes(p.id) ? 'border-teal-500 bg-teal-500/10 text-teal-400 shadow-[0_0_20px_#2dd4bf30]' : 'border-slate-800 text-slate-600'}`}>{p.name}</button>
                        ))}
                    </div>
