...
const CONFIG = {
    ...
    D: ["Unresolved Sexual Tension", "Grudging Respect", "Bitter Rivals", "Mentor / Mentee", "Buddy Cop", "Frenemies", "Strategic Alliance", "Shared Trauma Bond", "Competitive Peers", "Master / Apprentice"],
...
const App = () => {
    ...
    const testPing = async () => {
        ...
        } catch (e) { window.alert("HANDSHAKE FAIL: Reload PA Web Tab."); } finally { setLoad(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-2 text-teal-500 font-black text-xs uppercase"><Cpu size={14}/> Apex_v73.0</div>
                </div>
                <button onClick={testPing} className="p-3 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-teal-500 hover:text-black rounded transition-all"><Wifi size={12}/> Handshake</button>
            ...
            <main className="flex-1 flex flex-col p-10 overflow-y-auto relative bg-[#121416]">
                ...
                {!active ? (
                    <div className="grid grid-cols-2 gap-6 animate-in fade-in">
                        {((tab === 'season' ? seasons : personas) || []).map((i, k) => (
                            <div key={k} className="bg-[#1c1f23] border border-slate-800 p-6 rounded-2xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all" onClick={() => setActive(i)}>
                                ...
                                <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-lg italic truncate leading-none">{i.title || i.name}</h4><p className="text-[10px] text-teal-500 font-black opacity-60 uppercase mt-2">{i.rel || i.role || 'Record Found'}</p></div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="space-y-10">
                        ...
                        {tab === 'season' && (
                            <div className="space-y-10">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl"><h4 className="text-teal-400 text-[10px] font-black uppercase mb-4 italic tracking-widest"><ScrollText size={16}/> Blueprint</h4><p className="text-lg text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>
