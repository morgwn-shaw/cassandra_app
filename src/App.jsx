...
import { Zap, Trash2, Database, Quote, Loader2, UserPlus, History, FastForward, Wifi, Cpu, ThumbsUp, ThumbsDown, Dice5, PlayCircle, Globe, Calendar, RefreshCw, Terminal, X, Eye } from 'lucide-react';
...
const SafeBrief = ({ data }) => {
    ...
    return Object.entries(data).map(([act, text], idx) => (
        <div key={idx} className="mb-6 border-l-2 border-teal-900/30 pl-4 animate-in slide-in-from-left duration-300">
            <span className="text-[9px] text-teal-700 font-black uppercase">{act.replace('_', ' ')}</span>
            <p className="text-sm text-slate-400 italic leading-relaxed mt-1">"{text}"</p>
        </div>
    ));
};
...
const App = () => {
    ...
    useEffect(() => { sync(); }, [sync]);
    useEffect(() => { setBrief(null); }, [activeId, tab]);
    ...
    const showRaw = async () => {
        ...
        const d = await r.json(); setDebug(d.raw);
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            ...
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v139.0</div>
                    <div className="text-[9px] text-teal-900 font-black mt-1 uppercase italic"><Calendar size={10}/> March 25, 2026</div>
                </div>
                <div className="space-y-2">
                    ...
                    <button onClick={showRaw} className="w-full p-2 text-[9px] text-teal-900 font-black uppercase hover:text-teal-400 flex items-center justify-center gap-2 border border-dashed border-teal-950/40 rounded"><Terminal size={12}/> Forensic Feed</button>
                    <button onClick={() => setLoad(false)} className="w-full p-2 text-[9px] text-teal-900 font-black uppercase hover:text-teal-400 flex items-center justify-center gap-2"><RefreshCw size={12}/> Kill Blocker</button>
                </div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                ...
                <div className="flex gap-4 mb-10 border-b border-slate-800 pb-8">
                    <button onClick={() => {setActiveId(null); setTab('season');}} className={`px-12 py-4 text-[11px] font-black transition-all ${tab === 'season' && !activeId ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-800'}`}>SEASONS</button>
                    <button onClick={() => {setActiveId(null); setTab('persona');}} className={`px-12 py-4 text-[11px] font-black transition-all ${tab === 'persona' && !activeId ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-800'}`}>PERSONAS</button>
                </div>
                {!activeId ? (<div className="grid grid-cols-2 gap-8 animate-in fade-in">
                    {list.map((i, k) => (
                        <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all shadow-inner shadow-black/80" onClick={() => setActiveId(i.id)}>
                            ...
                            <img src={i?.portrait} className="w-20 h-20 rounded-2xl grayscale group-hover:grayscale-0 transition-all shadow-xl bg-black object-cover" alt="DNA" onError={(e) => { e.target.src = "https://via.placeholder.com/200?text=DNA_SYNCING"; }}/>
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-10"><h2 className="text-7xl font-black text-white uppercase italic tracking-tighter leading-none shadow-teal-500/5">{active?.title || active?.name}</h2><button onClick={() => setActiveId(null)} className="bg-teal-500 text-black px-12 py-4 text-[11px] font-black shadow-2xl active:scale-90 transition-all">BACK</button></div>
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> Forensic_Intelligence</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active?.summary || "Summary Generation Failed."}</p></div>
                            ...
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80 shadow-teal-500/5"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline decoration-teal-900 underline-offset-8 tracking-widest">Grounded_Audit</h4>
                                <SafeBrief data={brief} />
                                {brief && <div className="mt-10 pt-10 border-t border-teal-950/40"><button onClick={showRaw} className="text-[8px] text-teal-900 font-black uppercase hover:text-teal-500 flex items-center gap-2 italic"><Eye size={10}/> Peek Raw Payload</button></div>}
                            </div>
                        </div>)}
                        {tab === 'persona' && (<div className="grid grid-cols-2 gap-10 animate-in zoom-in-95"><div className="bg-black/40 p-12 border border-slate-800 rounded-3xl flex flex-col gap-6 h-[800px] overflow-y-auto shadow-inner shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase flex items-center gap-3 tracking-[0.3em]"><Quote size={20}/> Forensic Dossier</h4><p className="text-teal-600 text-[10px] font-black uppercase border-b border-slate-900 pb-2 italic">Role: {active?.role} | MBTI: {active?.archive?.mbti || 'NX'}</p><p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{active?.archive?.bio || 'DNA_SYNCING...'}</p></div><div className="bg-black/40 p-12 border border-slate-800 rounded-3xl h-[800px] overflow-y-auto custom-scrollbar shadow-2xl shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 tracking-[0.3em]">DNA_Memories (30)</h4>{(active?.archive?.anecdotes || []).map((a, j) => <p key={j} className="p-4 bg-white/5 border border-white/5 text-[10px] text-slate-500 italic mb-3 rounded-lg leading-relaxed uppercase shadow-sm">"{a}"</p>)}</div></div>)}
                    </div>
                )}
            </main>
