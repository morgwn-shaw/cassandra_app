...
const App = () => {
    ...
    const [activeId, setActiveId] = useState(null); // ID-based lookup for stability
    ...
    // LOOKUP DERIVATION: Prevents white-screen during background syncs
    const activeList = tab === 'season' ? seasons : personas;
    const active = activeList.find(item => item.id === activeId);
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest animate-pulse italic">Synchronizing Reality Signal...</p></div>}
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div className="bg-black/40 p-6 border border-slate-800 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[9px] font-black uppercase mb-4 flex items-center gap-2"><FastForward size={14}/> Future_Lore</h4><p className="text-[11px] text-slate-400 leading-relaxed italic">{active?.lore?.future_lore || "Syncing lore..."}</p></div>
                                </div>
