...
// --- MANUALLY VERIFY THIS URL ---
const PA_DOMAIN = "shadow-cassandrafiles.pythonanywhere.com";
const API_BASE = `https://${PA_DOMAIN}/api/v2`;
...
const App = () => {
    ...
    const [status, setStatus] = useState({ active_engine: 'Hunting...' });
    ...
    const runAction = async (endpoint, body) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(`Target: ${targetUrl}\n\nServer returned HTML/404. Check PA logs.`);
            }

            const result = await res.json();
            if (result.error) window.alert(`BACKEND_ERROR: ${result.error}`);
            await refreshData();
        } catch (e) {
            window.alert(`CRITICAL_FAULT:\n${e.message}`);
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0a0c0e] text-slate-400 select-none">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Zap size={16} /> Telemetry_v46.5</div>
                ...
                <div className="p-4 bg-slate-900/40 border border-slate-800 rounded">
                    <p className="text-[9px] text-teal-700 uppercase font-black mb-2">Endpoint_Verified</p>
                    <p className="text-[9px] text-slate-500 break-all">{API_BASE}</p>
                </div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {loading && (
                    <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase animate-pulse italic">Establishing_Neural_Sync...</p>
                    </div>
                )}
                ...
                {!activeItem ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                        {listData.map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-10 rounded-3xl cursor-pointer hover:border-teal-500 transition-all flex gap-8 items-center shadow-xl group relative" onClick={() => setActiveItem(item)}>
                                {viewMode === 'god' && <button onClick={(e) => deleteItem(e, activeTab, item.id)} className="absolute top-6 right-6 text-red-900 hover:text-red-500 z-10"><Trash2 size={18}/></button>}
                    ...
                    <div className="animate-in slide-in-from-bottom-8 duration-500 space-y-12">
                        <div className="flex justify-between items-end border-b border-slate-800 pb-10">
                            ...
                            <button onClick={() => setActiveItem(null)} className="bg-teal-500 text-black px-12 py-4 text-xs font-black shadow-2xl active:scale-95">CLOSE</button>
                        </div>
                        {activeTab === 'season' && (
                            <div className="flex flex-col gap-12">
                                ...
                                {viewMode === 'god' && (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 italic"><History size={16}/> Shared_Lore</h4>
                                            <div className="space-y-3 h-[300px] overflow-y-auto custom-scrollbar pr-4">
                                                {(activeItem?.lore?.shared_anecdotes || []).map((a, i) => <p key={i} className="text-[12px] text-slate-500 italic uppercase border-l-2 border-slate-900 pl-4 py-1 hover:text-white transition-all">"{a}"</p>)}
                                            </div>
                                        </div>
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 italic"><FastForward size={16}/> Seasonal_Roadmap</h4>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 border-t border-slate-900 pt-12">
                                    ...
                                    <div className="bg-black/60 border border-slate-800 rounded-3xl p-12 h-[900px] overflow-y-auto shadow-2xl relative">
                                        ...
                                        )) : <div className="h-full flex items-center justify-center text-slate-800 text-xs font-black uppercase tracking-[0.6em] opacity-20 italic text-center">Initialize_Node_Handshake</div>}
                                    </div>
                                </div>
                            </div>
                        )}
