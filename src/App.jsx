...
const App = () => {
    ...
    const runProduction = async (epIdx) => {
        ...
        try {
            for (let i = 1; i <= 6; i++) {
                ...
                if (!response.ok) throw new Error(data.error || `Act ${i} failed.`);
                
                // FORCE STRING EXTRACTION
                const actText = typeof data.script === 'object' ? JSON.stringify(data.script) : data.script;
                finalScript += `\n\n--- [ACT ${i}] ---\n\n` + actText;
                
                await new Promise(r => setTimeout(r, 800));
            }

            setStatus("SAVING MASTER SCRIPT...");
    ...
    const createSeason = async () => {
        setLoad(true); setStatus("ESTABLISHING ARC...");
        try {
            ...
            setStatus("SYNCING LORE...");
            await fetch(`${BASE_URL}/season/lore`, {
                method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ season_id: season.id })
            });
            await sync();
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[12px] select-none">
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="flex items-center gap-3 text-teal-500 font-black uppercase tracking-widest border-b border-teal-900/30 pb-4 italic"><Cpu size={18}/> Apex_v177</div>
            ...
            <main className="flex-1 flex flex-col p-10 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-8 border-b border-slate-800 pb-8 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); setActiveEp(null); }} className={`px-8 py-3 font-black text-[10px] uppercase transition-all rounded-lg ${!activeId ? 'bg-teal-500 text-black' : 'bg-slate-800 hover:bg-slate-700'}`}>Library</button>
                    {activeId && (
                        <>
                            ...
                            <button onClick={() => setViewLore(!viewLore)} className={`p-4 rounded-xl border flex items-center gap-2 uppercase text-[10px] font-black transition-all ${viewLore ? 'bg-teal-500 text-black border-teal-500' : 'border-slate-800 text-slate-500 hover:border-teal-500'}`}><History size={16}/> Lore</button>
                        </>
                    )}
                </div>
                    ...
                    <div className="flex flex-col h-full gap-8 overflow-hidden">
                        {viewLore ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-3xl p-10 overflow-y-auto custom-scrollbar space-y-6">
                                <h3 className="text-teal-500 font-black uppercase mb-6 border-b border-teal-900/20 pb-4 text-xl tracking-[0.2em] italic text-center text-center">Shared History</h3>
                                {activeSeason?.shared_history?.map((lore, i) => (
                                    <div key={i} className="p-8 bg-slate-900/50 border border-slate-800 rounded-3xl text-slate-300 uppercase text-[12px] leading-relaxed italic shadow-xl"><span className="text-teal-500 font-black mr-3">LOG_{i+1}:</span> {lore}</div>
                                ))}
                            </div>
                        ) : activeEp !== null ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-12 flex flex-col gap-8 overflow-hidden relative shadow-inner">
                                ...
                                <div className="space-y-3 border-b border-slate-800 pb-8 text-center">
                                    <h3 className="text-4xl font-black text-white uppercase italic tracking-tighter">{activeSeason.episodes[activeEp].title}</h3>
                                    <p className="text-teal-500 uppercase font-black text-[11px] tracking-[0.3em] italic">Atomic Producer Active</p>
                                </div>
                                <div className="flex-1 overflow-y-auto custom-scrollbar pr-6">
                                    {activeSeason.episodes[activeEp].full_script ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-full gap-8">
                                            ...
                                            <button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl hover:bg-white transition-all">Launch Producer</button>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">
                                {activeSeason.episodes.map((e, idx) => (
                                    <div key={idx} onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500/50 cursor-pointer transition-all shadow-xl group h-fit text-center">
            ...
            <aside className="w-[380px] bg-black/60 p-10 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-12 shadow-2xl custom-scrollbar">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <div className="space-y-6 p-8 bg-black/40 rounded-[2rem] border border-slate-800 shadow-inner">
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between text-[10px] font-black uppercase italic text-slate-500"><span>Runtime: {nS.target_runtime}m</span></div>
                    ...
                    <button onClick={createSeason} disabled={nS.host_ids.length !== 2 || !nS.topic} className="w-full py-6 bg-teal-500 text-black font-black uppercase text-[12px] rounded-[1.5rem] shadow-2xl">Establish Season</button>
                </div>
            </aside>
        </div>
    );
