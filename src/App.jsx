...
const App = () => {
    ...
    const runAction = async (path, body) => {
        ...
        try {
            ...
            window.alert("SIGNAL LOST: Server likely timed out during AI generation. Wait 30s and refresh."); 
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4"><div className="flex items-center gap-3 text-teal-500 font-black text-xs uppercase"><Cpu size={16}/> Apex_v79.0</div></div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-xl"><Loader2 className="animate-spin text-teal-500 mb-2"/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest">Generating Apex DNA...</p></div>}
                    ...
                    <div className="grid grid-cols-2 gap-8 animate-in fade-in">
                        {((tab === 'season' ? seasons : personas) || []).map((i, k) => (
                            <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all shadow-inner" onClick={() => setActive(i)}>
                                ...
                                <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-xl italic truncate">{i.title || i.name}</h4><p className="text-[10px] text-teal-500 font-black opacity-60 uppercase mt-2">{i.rel || i.role || 'Record Found'}</p></div>
                            </div>
                        ))}
                    </div>
                ) : (
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (
                            <div className="space-y-12">
                                <div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-3xl"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><ScrollText size={16}/> Grounded_Investigation</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>
            ...
            <aside className="w-[420px] bg-[#0b0c0e] p-12 flex flex-col gap-14 border-l border-slate-800 overflow-y-auto shrink-0 shadow-2xl">
                <div className="space-y-8">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <select className="bg-[#1c1f23] p-5 border border-slate-800 text-[11px] text-teal-500 font-black outline-none" value={nP.gender} onChange={(e) => setNP({...nP, gender: e.target.value})}>{CONFIG.G.map(g => <option key={g}>{g}</option>)}</select>
