...
const App = () => {
    ...
    const [load, setLoad] = useState(false); const [eng, setEng] = useState('SYNCING...');
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.model.replace('models/', '').toUpperCase()); window.alert("APEX SIGNAL ACTIVE: " + d.model);
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4"><div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v88.0</div><div className="text-[9px] text-teal-900 font-black mt-1 italic uppercase">{eng}</div></div>
                ...
                <div className="mt-auto pt-6 border-t border-slate-900"><button onClick={() => runAction('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black border border-red-900/30 hover:bg-red-600 transition-all uppercase">Reset_Vault</button></div>
            </aside>

            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-xl"><Loader2 className="animate-spin text-teal-500 mb-2"/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest">Grounding Apex Signal...</p></div>}
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><ScrollText size={16}/> Season_Summary</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>
                                <div className="space-y-4">
                                    {(active.episodes || []).map((e, idx) => (
                                        <div key={idx} className="bg-[#1c1f23] p-6 border border-slate-800 rounded-2xl flex justify-between items-center group hover:border-teal-500 transition-all shadow-lg">
                                            ...
                                            <button onClick={() => runAction('/showrunner/brief', { title: e.title, topic: active.title }).then(d => setBrief(d?.acts))} className="p-3 bg-slate-900 text-teal-500 rounded-full hover:bg-teal-500 hover:text-black transition-all shadow-xl active:scale-95"><PlayCircle size={20}/></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-0 shadow-2xl shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline decoration-teal-900 underline-offset-8">Production_Intelligence</h4>{brief ? Object.entries(brief).map(([act, text], idx) => (
                        ...
                        {tab === 'persona' && (<div className="grid grid-cols-2 gap-10 animate-in zoom-in-95"><div className="bg-black/40 p-12 border border-slate-800 rounded-3xl flex flex-col gap-6 h-[800px] overflow-y-auto shadow-inner shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase flex items-center gap-3 tracking-[0.3em]"><Quote size={20}/> Forensic Dossier</h4><p className="text-teal-600 text-xs font-black uppercase border-b border-slate-900 pb-2 italic">Role: {active.role} | MBTI: {active.archive?.mbti}</p><p className="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{active.archive?.bio}</p><div className="grid grid-cols-2 gap-6 border-t border-slate-800 pt-6"><div><h4 className="text-teal-600 text-[9px] font-black mb-4 uppercase flex items-center gap-1"><ThumbsUp size={12}/> Tangible_Likes</h4>{(active.archive?.likes || []).map((l, j) => <p key={j} className="text-[9px] text-teal-400 italic mb-1 uppercase">-{l}</p>)}</div><div><h4 className="text-red-900 text-[9px] font-black mb-4 uppercase flex items-center gap-1"><ThumbsDown size={12}/> Dislikes</h4>{(active.archive?.dislikes || []).map((d, j) => <p key={j} className="text-[9px] text-red-500 italic mb-1 uppercase">-{d}</p>)}</div></div></div><div className="bg-black/40 p-12 border border-slate-800 rounded-3xl h-[800px] overflow-y-auto custom-scrollbar shadow-inner shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 tracking-widest">DNA_Memories (30)</h4>{(active.archive?.anecdotes || []).map((a, j) => <p key={j} className="p-4 bg-white/5 border border-white/5 text-[10px] text-slate-500 italic mb-3 rounded-lg leading-relaxed uppercase shadow-sm">"{a}"</p>)}</div></div>)}
                    </div>
                )}
            </main>
            ...
            <aside className="w-[420px] bg-[#0b0c0e] p-12 flex flex-col gap-14 border-l border-slate-800 overflow-y-auto shrink-0 shadow-2xl">
                <div className="space-y-8"><h3 className="text-teal-400 text-[11px] font-black uppercase border-b border-slate-900 pb-4 tracking-widest"><UserPlus size={20}/> Identity_Spawn</h3>
                    ...
                    <input className="bg-[#1c1f23] p-5 border border-slate-800 text-[11px] text-slate-500 uppercase shadow-inner" placeholder="ROLE" value={nP.role} onChange={(e) => setNP({...nP, role: e.target.value})} /></div>
                    <div className="relative"><input className="w-full bg-[#1c1f23] p-5 border border-slate-800 text-[10px] text-slate-500 outline-none uppercase pr-14 shadow-inner" placeholder="CORE_TRAUMA" value={nP.trauma} onChange={(e) => setNP({...nP, trauma: e.target.value})} /><button onClick={() => setNP({...nP, trauma: CONFIG.T[Math.floor(Math.random() * CONFIG.T.length)]})} className="absolute right-4 top-4 text-slate-600 hover:text-teal-500 transition-all active:rotate-180 duration-300"><Dice5 size={24}/></button></div>
