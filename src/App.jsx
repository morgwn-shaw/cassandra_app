                ...
                {!active ? (<div className="grid grid-cols-2 gap-8 animate-in fade-in">
                    {(tab === 'season' ? (seasons || []) : (personas || [])).map((i, k) => (
                        <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all shadow-inner shadow-black/80" onClick={() => setActive(i)}>
                            ...
                            <img src={i?.portrait} className="w-20 h-20 rounded-2xl grayscale group-hover:grayscale-0 transition-all shadow-xl bg-black object-cover" alt="DNA" onError={(e) => { e.target.src = `https://via.placeholder.com/200?text=${i?.name || 'DNA'}`; }}/>
