...
const App = () => {
    ...
    const run = async (path, body) => {
        ...
        } catch (e) { window.alert("FETCH_FAIL: " + e.message); } finally { setLoad(false); }
    };

    const ping = async () => {
        setLoad(true); try {
            const r = await fetch("https://shadow-cassandrafiles.pythonanywhere.com/api/v2/ping");
            const d = await r.json(); window.alert("HANDSHAKE: " + d.status);
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden">
            <aside className="w-[280px] border-r border-slate-800 bg-black/60 p-6 flex flex-col gap-6 shrink-0 shadow-2xl">
                ...
                <button onClick={ping} className="p-3 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded"><Wifi size={12}/> Ping_Link</button>
                <div className="mt-auto space-y-4">
                    ...
                    <button onClick={() => run('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black uppercase border border-red-900/30 hover:bg-red-600 transition-all">Nuclear_Reset</button>
                </div>
            </aside>

            <main className="flex-1 flex flex-col p-10 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/90 z-[100] flex flex-col items-center justify-center"><Loader2 className="animate-spin text-teal-500 mb-2"/><p className="text-teal-500 text-[10px] font-black">SYNCING...</p></div>}
                ...
                {!active ? (
                    ...
                    <div className="space-y-10">
                        ...
                        {tab === 'persona' && (
                            <div className="grid grid-cols-2 gap-10"><div className="bg-black/40 p-10 border border-slate-800 rounded-3xl flex flex-col gap-8 h-[700px] overflow-y-auto"><h4 className="text-teal-500 text-[10px] font-black uppercase flex items-center gap-2 tracking-widest"><Quote size={16}/> Dossier</h4><p className="text-slate-300 text-sm leading-relaxed">{active.archive?.bio}</p><div className="grid grid-cols-2 gap-6 border-t border-slate-800 pt-8"><div><h4 className="text-teal-600 text-[9px] font-black uppercase mb-3"><ThumbsUp size={12}/> Likes</h4>{active.archive?.likes?.map((l, j) => <p key={j} className="text-[9px] text-teal-400 italic mb-2">-{l}</p>)}</div><div><h4 className="text-red-900 text-[9px] font-black uppercase mb-3"><ThumbsDown size={12}/> Hates</h4>{active.archive?.dislikes?.map((d, j) => <p key={j} className="text-[9px] text-red-500 italic mb-2">-{d}</p>)}</div></div></div><div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-[700px] overflow-y-auto custom-scrollbar"><h4 className="text-teal-500 text-[10px] font-black uppercase mb-6 tracking-widest">Memories</h4>{(active.archive?.anecdotes || []).map((a, j) => <p key={j} className="p-5 bg-white/5 text-[10px] text-slate-500 italic mb-4 rounded-xl leading-relaxed">"{a}"</p>)}</div></div>
                        )}
                    </div>
                )}
            </main>
