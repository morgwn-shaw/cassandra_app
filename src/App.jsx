...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, Terminal, Mic2, Headphones, Sparkles, UserCircle, ClipboardCheck, BarChart3, Copy } from 'lucide-react';

const BASE_URL = "https://shadow-cassandrafiles.pythonanywhere.com/api/v2";
const DESC_SAMPLES = ["A jaded specialist with a love for data.", "A former socialite turned radical environmentalist.", "A retired intelligence operative.", "A street-smart data broker."];
const CONFIG = {
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V191_RECONSTRUCTED", type: "system" }]);
        ...
        try {
            for (let i = 1; i <= 6; i++) {
                ...
                const response = await fetch(`${BASE_URL}/episode/act_script`, {
                    ...
                    body: JSON.stringify({ season_id: activeId.toString(), ep_idx: epIdx, act_num: i, previous_script: JSON.stringify(allBlocks.slice(-5)) })
                });
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0">
                <div className="p-4 border-b border-slate-800 flex items-center gap-2 text-teal-500 font-black uppercase text-[10px]"><Terminal size={14}/> Telemetry</div>
                        ...
                        {activeEp !== null ? (
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col overflow-hidden relative">
                                <div className="flex justify-between items-center mb-6 shrink-0">
                                    <div className="flex gap-6 text-[10px] font-black uppercase text-teal-500 bg-teal-500/5 px-6 py-2 rounded-full border border-teal-500/20 items-center">
                                        <BarChart3 size={14}/>
                                ...
                                {activeSeason?.episodes?.[activeEp]?.full_script_blocks ? (
                                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4">
                                        <textarea readOnly className="w-full h-96 bg-slate-950/80 p-8 rounded-3xl border border-teal-900/20 text-teal-300 font-mono text-[9px] resize-none outline-none" value={JSON.stringify(activeSeason.episodes[activeEp].full_script_blocks, null, 4)} />
                                        <div className="bg-teal-950/10 p-8 border border-teal-900/30 rounded-3xl">
                                            <h4 className="text-teal-500 font-black uppercase italic flex items-center gap-2 mb-4"><ClipboardCheck size={20}/> Assessment</h4>
                                            <p className="text-slate-400 uppercase text-[10px] leading-relaxed">{activeSeason.episodes[activeEp].assessment}</p>
                                        </div>
                                    </div>
                                ) : (
                            ...
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">
                                {activeSeason?.episodes?.map((e, idx) => (
                                    <div key={idx} onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500 cursor-pointer text-center h-fit group">
                                        ...
                                        {e.full_script_blocks && <div className="mt-4 text-teal-500 text-[10px] font-black uppercase italic flex items-center justify-center gap-2"><Copy size={12}/> json_locked</div>}
                                    </div>
                                ))}
                            </div>
                        )}
            ...
            <aside className="w-[340px] bg-black/60 p-8 border-l border-slate-800 overflow-y-auto shrink-0 flex flex-col gap-10 shadow-2xl custom-scrollbar">
                ...
                <div className="space-y-6 border-t border-slate-800 pt-10">
                    ...
                    <div className="space-y-6 p-8 bg-black/40 rounded-[2rem] border border-slate-800 shadow-inner">
                        ...
                        <input type="range" min="5" max="25" className="w-full accent-teal-500 h-1" value={nS.target_runtime} onChange={e => setNS({...nS, target_runtime: parseInt(e.target.value)})} />
                    </div>
            ...
            {viewPersona && (
                <div className="fixed inset-0 z-[600] bg-black/95 flex items-center justify-center p-12 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full max-w-5xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="w-[300px] shrink-0 space-y-6">
                            ...
                            <div className="p-4 bg-teal-950/20 border border-teal-900/30 rounded-xl font-black text-white text-[12px] uppercase">{viewPersona?.dna?.mbti || "INTJ"} Profile / {viewPersona?.voice_id}</div>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-8 uppercase text-[10px]">
                           <div className="p-8 bg-black/40 border border-slate-800 rounded-2xl select-text leading-relaxed italic">{viewPersona?.dna?.personality_blurb}</div>
