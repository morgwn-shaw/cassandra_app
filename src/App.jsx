...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V211_STABLE_READY", type: "system" }]);
                    ...
                    <div className="flex flex-col h-full gap-8 overflow-hidden">
                        ...
                        <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">{(activeSeason?.episodes || []).map((e, idx) => (
                            <div key={idx} className="relative group">
                                ...
                                <button onClick={(e) => { e.stopPropagation(); fetch(`${BASE_URL}/season/${activeId}/episode/${idx}`, {method:'DELETE'}).then(sync); }} className="absolute top-4 right-4 text-red-900 opacity-0 group-hover:opacity-100 transition-all hover:text-red-500"><Trash2 size={16}/></button>
                            </div>
                        ))}</div>
                    </div>
                )}
