...
const App = () => {
    ...
    const [load, setLoad] = useState(false); const [eng, setEng] = useState('FLASH_SYNCING...');
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.model.replace('models/', '').toUpperCase());
            window.alert("FLASH HANDSHAKE SUCCESS: USING " + d.model);
    ...
    const run = async (path, body, method = 'POST') => {
        ...
        } catch (e) { window.alert("TIMEOUT: Flash search pending. Refresh in 30s."); } finally { setLoad(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-xs uppercase tracking-widest"><Cpu size={16}/> Apex_v92.0</div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Engaging Flash Context...</p></div>}
                                ...
                                <div className="space-y-4">
                                    {(active.episodes || []).map((e, idx) => (
                                        <div key={idx} className="bg-[#1c1f23] p-6 border border-slate-800 rounded-2xl flex justify-between items-center group hover:border-teal-500 transition-all shadow-lg">
                                            <div><span className="text-[9px] text-teal-600 font-black uppercase">Node {e.node} - {e.phase}</span><h5 className="text-white font-bold italic uppercase leading-tight">{e.title}</h5></div>
