...
const CONFIG = {
    ...
    T: ["Witnessed server farm bleed-out.", "Neural-link betrayal.", "Identity wiped by ghost protocol.", "Exposed data laundering ring.", "Escaped digital cult."]
};
...
const App = () => {
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.status); window.alert("HANDSHAKE SUCCESS: " + d.status);
        } catch (e) { window.alert("HANDSHAKE FAILED: Visit the URL in your browser to check if PA is actually running."); } finally { setLoad(false); }
    };
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-6"><div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v73.0</div></div>
                ...
                <div className="mt-auto space-y-4">
                    <div className="flex border border-slate-800 rounded"><button onClick={() => setView('god')} className={`flex-1 py-2 text-[10px] font-black ${view === 'god' ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-900'}`}>GOD</button><button onClick={() => setView('user')} className={`flex-1 py-2 text-[10px] font-black ${view === 'user' ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-900'}`}>USER</button></div>
                    ...
                    <div className="grid grid-cols-2 gap-8 animate-in fade-in">
                        {((tab === 'season' ? seasons : personas) || []).map((i, k) => (
                            <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative active:scale-95 transition-all" onClick={() => setActive(i)}>
                                ...
                                <div className="min-w-0 flex-1"><h4 className="text-white font-black uppercase text-xl italic truncate leading-none">{i.title || i.name}</h4><p className="text-[10px] text-teal-500 font-black opacity-60 uppercase mt-2">{i.rel || i.role || 'Record Found'}</p></div>
                            </div>
                        ))}
                    </div>
                ) : (
