...
const App = () => {
    ...
    const [status, setStatus] = useState({ stage: 'IDLE', active_engine: 'Initializing...' });
    ...
    const runAction = async (endpoint, body) => {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(`Server returned HTML instead of JSON. Check PA Logs. (Snippet: ${text.substring(0, 50)})`);
            }

            const result = await response.json();
            if (result.error) {
                window.alert(`BACKEND_EXCEPTION: ${result.error}\n\nCheck Telemetry.`);
            }
            await refreshData();
        } catch (e) {
            window.alert(`CRITICAL_FAULT: ${e.message}`);
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0a0c0e] text-slate-400">
            ...
            <aside className="w-[320px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Zap size={16} /> Telemetry_v46.0</div>
                ...
                <div className="flex-1 overflow-y-auto space-y-4">
                    <div className="p-4 bg-slate-900/50 rounded border border-slate-800">
                        <p className="text-[10px] uppercase font-black text-teal-600 mb-1">Status</p>
                        <p className="text-white text-xs font-bold">{status.stage || "IDLE"}</p>
                    </div>
                </div>
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-900">
                    <div className="flex border border-slate-800 rounded overflow-hidden">
                        <button onClick={() => setViewMode('god')} className={`flex-1 py-2 text-[10px] font-black ${viewMode === 'god' ? 'bg-teal-500 text-black' : 'bg-slate-900'}`}>GOD_MODE</button>
                        <button onClick={() => setViewMode('user')} className={`flex-1 py-2 text-[10px] font-black ${viewMode === 'user' ? 'bg-teal-500 text-black' : 'bg-slate-900'}`}>USER_MODE</button>
                    </div>
                    <button onClick={() => runAction('/purge', {})} className="w-full p-4 bg-red-950/20 border border-red-900/30 text-red-500 text-[10px] font-black uppercase hover:bg-red-600 transition-all shadow-2xl flex items-center justify-center gap-2">
                        <ShieldAlert size={16}/> World_Purge
                    </button>
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {loading && (
                    <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-md">
                        ...
                        <p className="text-teal-400 text-xs font-black uppercase tracking-[1em] animate-pulse">Engaging_Apex_Neural_Handshake...</p>
                    </div>
                )}
                ...
                {!activeItem ? (
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in">
                        {((activeTab === 'season' ? seasons : personas) || []).map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-10 rounded-2xl cursor-pointer hover:border-teal-500 transition-all flex gap-8 items-center shadow-xl group relative active:scale-95" onClick={() => setActiveItem(item)}>
                        ...
                        {activeTab === 'season' && (
                            <div className="flex flex-col gap-12">
                                <div className="bg-teal-950/10 p-12 border border-teal-900/30 rounded-3xl shadow-2xl">
                                    <h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 flex items-center gap-3 italic tracking-[0.3em]"><ScrollText size={20}/> Geopolitical_Arc_Report</h4>
                                    <p className="text-[19px] text-slate-300 font-sans leading-relaxed uppercase selection:bg-teal-500/40">{activeItem?.summary}</p>
                                </div>

                                {viewMode === 'god' && (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><History size={18}/> Shared_Lore_Log</h4>
                                        ...
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            ...
                                            <p className="text-[15px] text-slate-400 uppercase leading-relaxed font-sans bg-[#1c1f23]/50 p-8 rounded-xl border border-slate-800/50 italic leading-snug">{activeItem?.lore?.future_lore || "No future sequence logged."}</p>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 border-t border-slate-900 pt-14">
                                        ...
                                        {(activeItem?.episodes || []).map((ep, idx) => (
                                            <div key={idx} className="p-8 bg-[#1c1f23] border border-slate-800 group hover:border-teal-500 transition-all flex flex-col gap-5 shadow-2xl rounded-2xl active:scale-[0.98]">
                                                <div className="flex justify-between items-center">
                                                    <span className="px-3 py-1 bg-slate-900 text-teal-500 text-[10px] font-black uppercase tracking-widest rounded-full border border-slate-800 italic">NODE_{ep?.act}</span>
                                    ...
                                    <div className="bg-black/60 border border-slate-800 rounded-3xl p-12 h-[900px] overflow-y-auto shadow-2xl relative">
                                        ...
                                        )) : <div className="h-full flex items-center justify-center text-slate-800 text-xs font-black uppercase tracking-[0.6em] opacity-20 italic">Awaiting_Neural_Sync</div>}
                                    </div>
                                </div>
                            </div>
                        )}
