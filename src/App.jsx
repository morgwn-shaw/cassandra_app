...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_FLOOR_V195_LOCKED", type: "system" }]);
    ...
    const [nP, setNP] = useState({ name: '', role: 'Host', gender: CONFIG.G[0], description: "A witty specialist with a deep archive." });
    ...
    const runProduction = async (epIdx) => {
        ...
        try {
            for (let i = 1; i <= 6; i++) {
                setStatus(`ACT ${i} (DYNAMIC LENGTH)...`);
        ...
        <div className=\"h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none\">
            ...
            <main className=\"flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden\">
                <div className=\"flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center\">
                    ...
                    {activeId && <div className=\"flex-1 text-teal-500 font-black uppercase italic truncate text-lg\"><ChevronRight className=\"inline mr-2\"/>{activeSeason?.title || \"Season\"}</div>}
                </div>
                            ...
                            <div className=\"flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col overflow-hidden relative\">
                                ...
                                {activeSeason?.episodes?.[activeEp]?.full_script_blocks ? (
                                    ...
                                    <div className=\"flex flex-col items-center justify-center h-full gap-8\">
                                        ...
                                        <button onClick={() => runProduction(activeEp)} className=\"px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl\">Start High-Verbosity Engine</button>
                                    </div>
                                )}
                            </div>
                        ) : (
