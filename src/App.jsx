...
const CONFIG = {
    ...
    T: ["Witnessed server farm bleed-out.", "Neural-link betrayal.", "Identity wiped by ghost protocol."]
};
...
const App = () => {
    ...
    const testPing = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); window.alert("HANDSHAKE SUCCESS: " + d.status);
        } catch (e) { window.alert("HANDSHAKE FAIL: " + e.message); } finally { setLoad(false); }
    };

    const runAction = async (path, body) => {
        setLoad(true); 
        const url = "https://shadow-cassandrafiles.pythonanywhere.com/api/v2" + path;
        try {
            const r = await fetch(url, { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify(body) });
            const d = await r.json(); 
            if (d.error) window.alert("AI_ERROR: " + d.error); 
            await sync();
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[280px] border-r border-slate-800 bg-black/60 p-6 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-2 text-teal-500 font-black text-xs uppercase"><Cpu size={14}/> Apex_v62.0</div>
                </div>
                <button onClick={testPing} className="p-3 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-teal-500 hover:text-black rounded"><Wifi size={12}/> Ping_Link</button>
