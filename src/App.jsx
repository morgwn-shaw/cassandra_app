...
const App = () => {
    ...
    const [status, setStatus] = useState({ active_engine: 'Apex_Standby...' });
        ...
        <div className="h-screen w-screen font-mono flex overflow-hidden bg-[#0a0c0e] text-slate-400 select-none">
            ...
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shadow-2xl shrink-0">
                <div className="border-b border-slate-900 pb-6">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16} /> Apex_v52.0</div>
                ...
                <button onClick={testConnection} className="w-full p-4 border border-teal-900/30 bg-teal-500/5 text-teal-500 text-[10px] font-black uppercase flex items-center justify-center gap-2 hover:bg-teal-500 hover:text-black transition-all rounded shadow-lg">
                    <Wifi size={14}/> Test_Apex_Handshake
                </button>
                        ...
                        {((activeTab === 'season' ? seasons : personas) || []).map((item, i) => (
                            <div key={i} className="bg-[#1c1f23] border border-slate-800 p-10 rounded-2xl cursor-pointer hover:border-teal-500 transition-all flex gap-8 items-center shadow-xl group relative" onClick={() => setActiveItem(item)}>
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="text-white font-black uppercase text-xl italic truncate tracking-tighter leading-none">{item?.title || item?.name}</h4>
                            ...
                            <div className="flex flex-col gap-12">
                                ...
                                {viewMode === 'god' && (
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><History size={16}/> Shared_Forensic_History</h4>
                                            <div className="space-y-3 h-[400px] overflow-y-auto custom-scrollbar pr-4">
                                                {(activeItem?.lore?.shared_anecdotes || []).map((a, i) => <p key={i} className="text-[11px] text-slate-500 italic uppercase border-l-2 border-slate-900 pl-6 py-2 hover:text-white transition-all">"{a}"</p>)}
                                            </div>
                                        </div>
                                        <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl">
                                            <h4 className="text-teal-400 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic"><FastForward size={16}/> Relationship_Evolution</h4>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 border-t border-slate-900 pt-14">
                                    <div className="space-y-6">
                                        {(activeItem?.episodes || []).map((ep, idx) => (
                                            <div key={idx} className="p-8 bg-[#1c1f23] border border-slate-800 group hover:border-teal-500 transition-all flex flex-col gap-4 shadow-xl rounded-2xl active:scale-[0.98]">
                                                ...
                                                <h4 className="text-white font-black text-2xl uppercase italic leading-none tracking-tighter">{ep?.title}</h4>
                                    ...
                                    <div className="bg-black/60 border border-slate-800 rounded-3xl p-12 h-[900px] overflow-y-auto shadow-2xl relative">
                                        <h4 className="text-teal-500 text-[12px] font-black uppercase mb-10 border-b border-slate-900 pb-6 flex items-center gap-4"><ShieldCheck size={24}/> Production_Brief [{activeBrief?.archetype || "Standby"}]</h4>
