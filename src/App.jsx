...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, Terminal, Mic2, Headphones, Sparkles, UserCircle, ClipboardCheck } from 'lucide-react';
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V189_FULL_RESTORATION", type: "system" }]);
    ...
    const createPersona = async () => {
        ...
        addLog(`INIT_DNA: ${nP.name}`);
        try {
            await fetch(`${BASE_URL}/persona/create`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify(nP) });
            await sync();
            setNP({ ...nP, name: '', description: DESC_SAMPLES[Math.floor(Math.random()*DESC_SAMPLES.length)] });
        } finally { setLoad(false); setStatus(""); }
    };

    const createSeason = async () => {
        if (!nS.topic || nS.host_ids.length < 2) return addLog("INVALID_FLIGHT: Missing Topic or Hosts", "error");
        setLoad(true);
        try {
    ...
    const runProduction = async (epIdx) => {
        ...
        try {
            for (let i = 1; i <= 6; i++) {
                ...
                finalScript += (i > 1 ? "\n\n" : "") + data.script;
                addLog(`ACT_${i}_READY`, "llm");
                await new Promise(r => setTimeout(r, 1000));
            }
            setStatus("SHOWRUNNER REVIEW...");
            const resAss = await fetch(`${BASE_URL}/episode/assess`, { method: 'POST', headers: {'Content-Type':'application/json'}, body: JSON.stringify({ season_id: activeId.toString(), ep_idx: epIdx, full_script: finalScript }) });
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <aside className="w-[260px] border-r border-slate-800 bg-black/80 flex flex-col shrink-0">
                ...
                <div ref={logRef} className="flex-1 overflow-y-auto p-4 space-y-2 bg-black/20 custom-scrollbar text-[9px] uppercase">
                    {logs.map((log, i) => <div key={i} className={log.type === 'error' ? 'text-red-500' : 'opacity-60'}>[{log.t}] {log.m.substring(0, 50)}</div>)}
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); setActiveEp(null); }} className={`px-8 py-3 font-black uppercase rounded-lg ${!activeId ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-800 hover:bg-slate-700'}`}>Library</button>
                    {activeId && (
                        <>
                            <div className="flex-1 text-teal-500 font-black uppercase italic truncate text-lg tracking-widest px-4"><ChevronRight className="inline mr-2"/>{activeSeason?.title || "SYNCHRONIZING..."}</div>
                        ...
                        {activeEp !== null ? (
                                ...
                                <div className="space-y-10">
                                    {activeSeason?.episodes?.[activeEp]?.full_script ? (
                                        ...
                                        <div className="flex flex-col items-center justify-center h-[50vh] gap-8">
                                            ...
                                            <button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl">Produce Sequence</button>
                                        </div>
                                    )}
                                </div>
                            </div>
                            ...
                            <div className="flex-1 bg-black/30 border border-slate-800 rounded-3xl p-10 overflow-y-auto custom-scrollbar space-y-6">
                                <div className="p-6 bg-teal-950/20 border border-teal-900/30 rounded-2xl">
                                    <h4 className="text-teal-500 font-black uppercase italic mb-2"><Zap size={16} className="inline mr-2"/> Future Lore Mapping</h4>
                                    <p className="text-slate-300 uppercase leading-relaxed text-[10px]">{activeSeason?.future_lore || "Syncing..."}</p>
                                </div>
                                {(activeSeason?.shared_history || []).map((lore, i) => (
                                    <div key={i} className="p-8 bg-slate-900/50 border border-slate-800 rounded-3xl text-slate-300 uppercase text-[11px] leading-relaxed italic"><span className="text-teal-500 font-black mr-3">LOG_{i+1}:</span> {lore}</div>
                                ))}
                            </div>
                        ) : (
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto custom-scrollbar pr-4">
                                {activeSeason?.episodes?.map((e, idx) => (
                                    <div key={idx} onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500 cursor-pointer transition-all shadow-xl text-center h-fit">
                                        ...
                                        {e.full_script && <div className="mt-4 text-teal-500 text-[10px] font-black uppercase italic flex items-center justify-center gap-2"><Zap size={12}/> Script Saved</div>}
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <div className="fixed inset-0 z-[600] bg-black/95 flex items-center justify-center p-12 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        <div className="w-[300px] shrink-0 space-y-6">
                            <img src={viewPersona?.portrait} className="w-full aspect-square rounded-2xl border-2 border-teal-900/20 bg-black shadow-xl" />
                            <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">{viewPersona?.name}</h2>
                            <div className="p-4 bg-teal-950/20 border border-teal-900/30 rounded-xl font-black text-white text-[12px] uppercase">
                                {viewPersona?.dna?.mbti || "INTJ"} Type / {viewPersona?.voice_id}
                            </div>
