...
import { Zap, Trash2, Database, Quote, Loader2, UserPlus, History, FastForward, Wifi, Cpu, ThumbsUp, ThumbsDown, Dice5, ScrollText, PlayCircle, Globe, Calendar } from 'lucide-react';
...
const App = () => {
    ...
    const [brief, setBrief] = useState(null); const [curDate, setCurDate] = useState('2026-03-24');
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.model.toUpperCase()); setCurDate(d.date);
    ...
    const runAction = async (path, body, method = 'POST') => {
        ...
        } catch (e) { window.alert("SIGNAL TIMEOUT: Deep search in progress. Wait 60s."); } finally { setLoad(false); }
    };
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v90.0</div>
                    <div className="text-[9px] text-teal-900 font-black mt-1 uppercase flex items-center gap-2 italic"><Calendar size={10}/> Grounded: {curDate}</div>
                </div>
                ...
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-900">
                    <button onClick={() => runAction('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black border border-red-900/30 hover:bg-red-600 transition-all uppercase">Purge_Vault</button>
                </div>
            </aside>

            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest">Compiling Multi-Pass Intelligence...</p></div>}
                ...
                {!active ? (<div className="grid grid-cols-2 gap-8">
                    {((tab === 'season' ? seasons : personas) || []).map((i, k) => (
                        <div key={k} className="bg-[#1c1f23] border border-slate-800 p-8 rounded-3xl cursor-pointer hover:border-teal-500 flex gap-6 items-center group relative transition-all" onClick={() => setActive(i)}>
                            ...
                            <img src={i.portrait} className="w-20 h-20 rounded-2xl grayscale group-hover:grayscale-0 transition-all" alt="D"/>
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> Grounded_Report</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>
                            ...
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline underline-offset-8">Production_Brief</h4>{brief ? Object.entries(brief).map(([act, text], idx) => (
            ...
            <aside className="w-[420px] bg-[#0b0c0e] p-12 flex flex-col gap-14 border-l border-slate-800 overflow-y-auto shrink-0 shadow-2xl">
                <div className="space-y-8"><h3 className="text-teal-400 text-[11px] font-black uppercase border-b border-slate-900 pb-4 tracking-widest"><UserPlus size={20}/> Identity_Spawn</h3>
                    ...
                    <button onClick={() => runAction('/persona/create', nP)} disabled={load || !nP.name} className="w-full py-6 bg-teal-500 text-black text-[11px] font-black uppercase shadow-2xl hover:bg-white active:scale-95 transition-all shadow-teal-500/20 italic">Commit_DNA</button>
                </div>
