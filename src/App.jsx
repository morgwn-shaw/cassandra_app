...
import { Zap, Activity, Code, Cpu, Layers, Fingerprint, Trash2, Plus, PlayCircle, UserPlus, Database, Quote, X, AlertCircle } from 'lucide-react';
...
const App = () => {
  ...
  const [activeItem, setActiveItem] = useState(null); // The selected Season or Persona
  ...
  // Polling with Stability Control
  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await fetch(`${API_BASE}/status`);
        const data = await res.json();
        setLiveStatus(prev => ({ ...data, history: data.history })); // Prevent unnecessary re-renders
      } catch (e) {}
    }, 1500);
  ...
  return (
    <div className="h-screen w-screen bg-[#020202] text-zinc-400 font-mono flex overflow-hidden">
      ...
      {/* PANE 1: GOD MODE TELEMETRY (LEFT) */}
      <section className="w-[450px] border-r border-zinc-900 bg-black flex flex-col p-6 shadow-2xl overflow-hidden">
        <div className="flex items-center gap-3 text-[#00ffcc] mb-8 uppercase text-[10px] font-black tracking-[0.4em]">
            <Zap size={14} className="animate-pulse" /> Shadow_Kernel_v24
        </div>

        <div className="flex-1 flex flex-col gap-4 overflow-hidden">
            <div className="p-4 bg-zinc-950 border border-zinc-900 rounded flex flex-col">
                ...
                <div className="h-56 overflow-y-auto space-y-1 pr-2 custom-scrollbar text-[10px]">
                    {liveStatus.history.map((log, i) => (
                        <div key={i} className="text-zinc-600 border-l border-zinc-900 pl-3 py-1 font-bold whitespace-nowrap overflow-hidden text-ellipsis">{log}</div>
                    ))}
                    <div ref={logEndRef} />
                </div>
            </div>
            ...
            <div className="p-4 bg-zinc-950 border border-zinc-900 rounded flex-1 flex flex-col overflow-hidden">
                ...
                <pre className="text-[9px] text-zinc-700 bg-black p-3 rounded flex-1 overflow-auto whitespace-pre-wrap leading-tight">{liveStatus.request_payload}</pre>
            </div>

            <div className="p-4 bg-zinc-950 border border-zinc-900 rounded flex-1 flex flex-col overflow-hidden">
                <p className="text-blue-400 text-[9px] font-black uppercase mb-3 flex items-center gap-2"><Cpu size={12}/> Engine_Response</p>
                <pre className="text-[9px] text-zinc-700 bg-black p-3 rounded flex-1 overflow-auto whitespace-pre-wrap leading-tight">{liveStatus.response_payload}</pre>
            </div>
        </div>
      </section>
      ...
      <section className="flex-1 flex flex-col bg-[#080808] p-10 overflow-y-auto border-r border-zinc-900">
        <div className="flex gap-4 mb-10 border-b border-zinc-900 pb-6">
            <button onClick={() => { setActiveTab('season'); setActiveItem(null); }} className={`px-8 py-3 text-[10px] font-black uppercase border transition-all ${activeTab === 'season' && !activeItem ? 'bg-[#00ffcc] text-black border-[#00ffcc]' : 'text-zinc-600 border-zinc-900'}`}>Season_Arcs</button>
            <button onClick={() => { setActiveTab('persona'); setActiveItem(null); }} className={`px-8 py-3 text-[10px] font-black uppercase border transition-all ${activeTab === 'persona' && !activeItem ? 'bg-[#00ffcc] text-black border-[#00ffcc]' : 'text-zinc-600 border-zinc-900'}`}>Persona_Vault</button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {activeTab === 'season' && seasons.map((s, i) => (
                  <div key={i} className="bg-black border border-zinc-900 p-8 rounded relative group hover:border-[#00ffcc]/30 transition-all cursor-pointer" onClick={() => setActiveItem(s)}>
                      ...
                      <p className="text-[10px] text-zinc-600 italic uppercase mb-6 tracking-widest">{s.relationship} // {s.episodes?.length || 0} NODES</p>
                      <div className="w-full py-3 bg-zinc-900 text-[#00ffcc] text-[9px] font-black uppercase flex items-center justify-center gap-3 border border-zinc-800 group-hover:bg-[#00ffcc] group-hover:text-black transition-all">Open_Blueprint</div>
                  </div>
              ))}
              {activeTab === 'persona' && personas.map((p, i) => (
                  <div key={i} className="bg-black border border-zinc-900 p-8 rounded relative group hover:border-[#00ffcc]/30 transition-all cursor-pointer" onClick={() => setActiveItem(p)}>
                      ...
                      <p className="text-[9px] text-zinc-800 font-bold border-t border-zinc-900 pt-4 truncate uppercase">{p.trauma}</p>
                  </div>
              ))}
          </div>
        )}

        {/* DETAIL VIEW: SEASON DEEP DIVE */}
        {activeItem && activeTab === 'season' && (
            <div className="animate-in fade-in duration-500 space-y-12">
                <div className="flex justify-between items-center border-b border-zinc-900 pb-8">
                    <h2 className="text-4xl font-black text-[#00ffcc] uppercase italic">{activeItem.title}</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="bg-black/40 border border-zinc-900 p-10 rounded-xl space-y-8">
                        <h4 className="text-zinc-600 text-[10px] font-black uppercase border-b border-zinc-900 pb-2 italic">Shared_Lore_Registry</h4>
                        <div className="space-y-6">
                            {(activeItem.shared_lore || []).map((l, i) => (
                                <p key={i} className="text-[12px] text-[#ff6666] font-mono uppercase border-l border-red-900/30 pl-4 italic">"{l}"</p>
                            ))}
                        </div>
                    </div>
                    <div className="bg-black/40 border border-zinc-900 p-10 rounded-xl space-y-8">
                        ...
                        <div className="space-y-3">
                            {(activeItem.episodes || []).map((ep, idx) => (
                                <div key={idx} className="p-4 border border-zinc-800 bg-zinc-950 flex flex-col">
                                    ...
                                    <p className="text-[9px] text-zinc-500 italic mt-1 uppercase tracking-tighter">{ep.sub_topic}</p>
                                </div>
                            ))}
                        </div>
                    </div>
        ...
        {/* DETAIL VIEW: PERSONA DNA DIVE */}
        {activeItem && activeTab === 'persona' && (
            <div className="animate-in fade-in duration-500 space-y-12 h-full flex flex-col">
                <div className="flex justify-between items-center border-b border-zinc-900 pb-8">
                    <h2 className="text-4xl font-black text-[#00ffcc] uppercase italic">{activeItem.name}</h2>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 flex-1 overflow-hidden">
                    ...
                    <div className="bg-black/40 border border-zinc-900 p-10 rounded-xl flex flex-col overflow-hidden">
                        <h4 className="text-zinc-600 text-[10px] font-black uppercase border-b border-zinc-900 pb-4 mb-6 italic tracking-widest">Memories_&_Anecdotes</h4>
      ...
      <section className="w-[450px] bg-black p-10 flex flex-col gap-12 overflow-y-auto">
        
        {/* SPAWN PERSONA */}
        <div className="space-y-6 border border-zinc-900 p-8 rounded-2xl bg-zinc-950/20">
            <h3 className="text-[#00ffcc] text-[10px] font-black uppercase flex items-center gap-2 tracking-widest mb-4"><UserPlus size={14}/> Spawn_Identity</h3>
            <div className="space-y-4">
                <input className="w-full bg-zinc-900 p-4 border border-zinc-800 text-[11px] text-[#00ffcc] font-bold outline-none" placeholder="IDENTITY_NAME" value={newP.name} onChange={(e) => setNewP({...newP, name: e.target.value})} />
                <input className="w-full bg-zinc-900 p-4 border border-zinc-800 text-[11px] text-zinc-500 outline-none uppercase" placeholder="CORE_TRAUMA" value={newP.trauma} onChange={(e) => setNewP({...newP, trauma: e.target.value})} />
                <button onClick={spawnPersona} disabled={loading || !newP.name} className={`w-full py-4 text-[10px] font-black uppercase transition-all rounded-lg ${loading || !newP.name ? 'bg-zinc-950 text-zinc-800 opacity-20 cursor-not-allowed' : 'bg-[#00ffcc] text-black shadow-lg shadow-[#00ffcc]/10 hover:bg-white'}`}>Commit_DNA</button>
            </div>
        </div>

        {/* ESTABLISH ARC */}
        <div className="space-y-6 border border-zinc-900 p-8 rounded-2xl bg-zinc-950/20">
            <h3 className="text-[#00ffcc] text-[10px] font-black uppercase flex items-center gap-2 tracking-widest mb-4"><Database size={14}/> Establish_Arc</h3>
            <div className="space-y-8">
                ...
                <div className="space-y-3">
                    <p className="text-zinc-600 text-[9px] font-black uppercase ml-1">DNA_Pairing (Select 2)</p>
                ...
                <div className="space-y-3">
                    <p className="text-zinc-600 text-[9px] font-black uppercase ml-1">Node_Count: {newS.episodes_count}</p>
                ...
                {loading && (
                    <div className="flex items-center justify-center gap-3 text-zinc-500 animate-pulse text-[9px] font-black uppercase">
                        <Activity size={12}/> Processing Engine Handshake...
                    </div>
                )}
            </div>
        </div>
