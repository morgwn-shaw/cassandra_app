...
const App = () => {
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            const d = await r.json(); setEng(d.model.toUpperCase()); window.alert("APEX CONNECTED: " + d.model);
    ...
    const run = async (path, body, method = 'POST') => {
        setLoad(true); try {
            ...
            const d = await r.json(); if (d.error) window.alert(d.error); await sync(); return d;
        } catch (e) { window.alert("TIMEOUT: GROUNDED SEARCH PENDING"); } finally { setLoad(false); }
    };
    ...
    return (
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4"><div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v89.0</div><div className="text-[9px] text-teal-900 font-black mt-1 uppercase italic">{eng}</div></div>
                <button onClick={ping} className="p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all"><Wifi size={14}/> Handshake</button>
                <div className="mt-auto pt-6 border-t border-slate-900"><button onClick={() => run('/purge', {})} className="w-full p-3 bg-red-950/20 text-red-500 text-[10px] font-black border border-red-900/30 hover:bg-red-600 transition-all uppercase"><Zap size={14}/> Purge Vault</button></div>
            </aside>

            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest animate-pulse">Establishing Signal...</p></div>}
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[11px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> Season_Blueprint</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active.summary}</p></div>
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-black/40 p-6 border border-slate-800 rounded-3xl"><h4 className="text-teal-500 text-[9px] font-black uppercase mb-4 flex items-center gap-2"><History size={14}/> Shared_Lore</h4>{(active.lore?.shared_anecdotes || []).slice(0,10).map((a, j) => <p key={j} className="text-[10px] text-slate-500 italic mb-2 leading-tight">"{a}"</p>)}</div>
                                    <div className="bg-black/40 p-6 border border-slate-800 rounded-3xl"><h4 className="text-teal-400 text-[9px] font-black uppercase mb-4 flex items-center gap-2"><FastForward size={14}/> Future_Lore</h4><p className="text-[11px] text-slate-400 leading-relaxed italic">{active.lore?.future_lore}</p></div>
                                </div>
                                <div className="space-y-4">
                                    {(active.episodes || []).map((e, idx) => (
                                        <div key={idx} className="bg-[#1c1f23] p-6 border border-slate-800 rounded-2xl flex justify-between items-center group hover:border-teal-500 transition-all shadow-lg">
                                            <div><span className="text-[9px] text-teal-600 font-black uppercase">Node {e.node} - {e.phase}</span><h5 className="text-white font-bold italic uppercase">{e.title}</h5></div>
                                            <button onClick={() => run('/showrunner/brief', { title: e.title, phase: e.phase, season_id: active.id }).then(d => setBrief(d?.acts))} className="p-3 bg-slate-900 text-teal-500 rounded-full hover:bg-teal-500 hover:text-black transition-all shadow-xl active:scale-95"><PlayCircle size={20}/></button>
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <div className="bg-black/40 p-10 border border-slate-800 rounded-3xl h-fit sticky top-4 shadow-2xl shadow-black/80 shadow-teal-500/5"><h4 className="text-teal-500 text-[11px] font-black uppercase mb-6 flex items-center gap-2 italic underline underline-offset-8">Episode_Intelligence</h4>{brief ? Object.entries(brief).map(([act, text], idx) => (
            ...
            <aside className="w-[420px] bg-[#0b0c0e] p-12 flex flex-col gap-14 border-l border-slate-800 overflow-y-auto shrink-0 shadow-2xl shadow-black/80">
                <div className="space-y-8"><h3 className="text-teal-400 text-[11px] font-black uppercase border-b border-slate-900 pb-4 tracking-widest"><UserPlus size={20}/> Identity_Spawn</h3>
                    ...
                    <button onClick={() => run('/persona/create', nP)} disabled={load || !nP.name} className="w-full py-6 bg-teal-500 text-black text-[11px] font-black uppercase shadow-2xl hover:bg-white active:scale-95 transition-all shadow-teal-500/20">Commit DNA</button>
                </div>
                <div className="space-y-8 border-t border-slate-900 pt-14"><h3 className="text-teal-400 text-[11px] font-black uppercase border-b border-slate-900 pb-4 tracking-widest"><Database size={20}/> Establish Season</h3>
                    ...
                    <div className="space-y-4">
                        ...
                        <input type="range" min="1" max="24" className="w-full accent-teal-500 bg-slate-900 h-2 rounded-lg appearance-none cursor-pointer shadow-inner transition-all" value={nS.episodes_count} onChange={(e) => setNS({...nS, episodes_count: e.target.value})} />
                    </div>
