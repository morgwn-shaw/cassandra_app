...
import { Trash2, UserPlus, Wifi, Cpu, X, Archive, BookOpen, History, ChevronRight, FileText, Activity, Zap, Terminal, Mic2, Headphones, Sparkles, UserCircle, MessageSquare, Copy, BarChart3, ClipboardCheck, Info, Volume2 } from 'lucide-react';
...
const App = () => {
    ...
    const [logs, setLogs] = useState([{ t: new Date().toLocaleTimeString(), m: "APEX_V201_GOSPEL_ANCHOR", type: "system" }]);
    ...
    const [nP, setNP] = useState({ name: '', role: 'Host', gender: CONFIG.G[0], description: "Specialist.", trauma: '' });
    ...
    const createSeason = async () => {
        ...
        setLoad(true); try {
            ...
            const s = await r1.json(); setStatus("Forensic Research...");
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden text-[11px] select-none">
            ...
            <main className="flex-1 flex flex-col p-8 bg-[#0d0f11] relative overflow-hidden">
                <div className="flex gap-4 mb-6 border-b border-slate-800 pb-6 shrink-0 items-center">
                    <button onClick={() => { setActiveId(null); setViewLore(false); setActiveEp(null); setAudioManifest(null); }} className={`px-8 py-3 font-black uppercase rounded-lg ${!activeId ? 'bg-teal-500 text-black shadow-lg' : 'bg-slate-800'}`}>Library</button>
                    {activeId && <><div className="flex-1 text-teal-500 font-black uppercase italic truncate text-lg px-4"><ChevronRight className="inline mr-2"/>{activeSeason?.title}</div><button onClick={() => setViewLore(!viewLore)} className={`p-4 rounded-xl border flex items-center gap-2 uppercase font-black transition-all ${viewLore ? 'bg-teal-500 text-black border-teal-500' : 'border-slate-800 text-slate-500'}`}><History size={16}/> Lore</button></>}
                </div>
                ...
                {!activeId ? (
                    ...
                    <div className="flex-1 bg-black/30 border border-slate-800 rounded-[2.5rem] p-8 flex flex-col overflow-hidden relative shadow-2xl">
                        <div className="flex justify-between items-center mb-6 shrink-0">
                            <div className="flex gap-4"><button onClick={() => { setActiveEp(null); setAudioManifest(null); }} className="text-slate-500 hover:text-white uppercase font-black italic text-[10px]">[ Close ]</button>{!audioManifest && activeSeason.episodes[activeEp].full_script_blocks && <button onClick={generateAudio} className="px-6 py-2 bg-teal-500 text-black font-black uppercase rounded-lg text-[9px] flex items-center gap-2 shadow-xl"><Volume2 size={12}/> Synthesize Audio</button>}</div>
                            <div className="flex gap-6 text-[10px] font-black uppercase italic text-teal-500 bg-teal-500/5 px-6 py-2 rounded-full border border-teal-500/20 items-center"><BarChart3 size={14}/><span>Chars: {getStats(activeSeason?.episodes?.[activeEp]?.full_script_blocks).chars}</span><span>EST: {getStats(activeSeason?.episodes?.[activeEp]?.full_script_blocks).time}m</span></div>
                        </div>
                        <div className="flex-1 overflow-y-auto custom-scrollbar space-y-4 pr-2">{audioManifest ? (
                            <div className="space-y-3">{audioManifest.map((a, i) => (
                                <div key={i} className="p-4 bg-slate-900/50 border border-slate-800 rounded-2xl flex items-center gap-6 hover:border-teal-500 transition-all group">
                                    ...
                                    <div className="flex-1"><div className="text-[9px] text-teal-600 font-black uppercase mb-1">{a.name}</div><div className="text-[10px] text-slate-300 leading-relaxed italic">{a.text}</div></div>
                                    <audio controls className="h-8 accent-teal-500 w-48"><source src={`https://shadow-cassandrafiles.pythonanywhere.com${a.url}`} type="audio/mpeg"/></audio>
                                </div>
                            ))}</div>
                        ) : activeSeason?.episodes?.[activeEp]?.full_script_blocks ? (
                            <><textarea readOnly className="w-full h-[40vh] bg-slate-950/80 p-8 rounded-3xl border border-teal-900/20 text-teal-300 font-mono text-[9px] resize-none outline-none select-text custom-scrollbar" value={JSON.stringify(activeSeason.episodes[activeEp].full_script_blocks, null, 4)} /><div className="bg-teal-950/10 p-8 border border-teal-900/30 rounded-3xl shadow-xl"><h4 className="text-teal-500 font-black uppercase italic flex items-center gap-2 mb-4"><ClipboardCheck size={20}/> Post-Production Review</h4><p className="text-slate-300 uppercase text-[10px] leading-relaxed">{activeSeason.episodes[activeEp].assessment}</p></div></>
                        ) : <div className="flex flex-col items-center justify-center h-full gap-8 opacity-40"><Mic2 size={80}/><button onClick={() => runProduction(activeEp)} className="px-12 py-5 bg-teal-500 text-black font-black uppercase text-[12px] rounded-2xl shadow-xl">Launch JSON Engine</button></div>}</div>
                    </div>
                ) : (
                    <div className="flex flex-col h-full gap-8 overflow-hidden">{viewLore ? (
                        <div className="flex-1 bg-black/30 border border-slate-800 rounded-3xl p-10 overflow-y-auto custom-scrollbar space-y-6">
                            <div className="p-6 bg-teal-950/20 border border-teal-900/30 rounded-2xl mb-6 shadow-xl"><h4 className="text-teal-500 font-black uppercase italic mb-2 flex items-center gap-2"><Zap size={16}/> Relationship Projection</h4><p className="text-slate-300 uppercase leading-relaxed text-[10px]">{activeSeason?.future_lore || "No projection mapped."}</p></div>
                            {(activeSeason?.shared_history || []).map((lore, i) => (<div key={i} className="p-8 bg-slate-900/50 border border-slate-800 rounded-3xl text-slate-300 text-[11px] italic uppercase shadow-xl hover:border-teal-900/50 transition-all"><span className="text-teal-500 font-black mr-3">LOG_{i+1}:</span> {lore}</div>))}
                        </div>
                    ) : (
                        <div className="flex flex-col h-full gap-8 overflow-hidden">
                            <div className="bg-slate-900/50 p-10 border border-slate-800 rounded-[2.5rem] shadow-2xl relative overflow-hidden group"><div className="absolute top-0 right-0 p-8 text-teal-900 group-hover:text-teal-500 transition-colors"><Info size={40}/></div><h3 className="text-teal-500 font-black uppercase italic tracking-widest mb-6 flex items-center gap-3"><BookOpen size={20}/> Factual Season Briefing</h3><p className="text-slate-200 text-lg leading-relaxed uppercase select-text italic relative z-10">{activeSeason?.description || "Researching factual context..."}</p></div>
                            <div className="grid grid-cols-3 gap-6 overflow-y-auto pr-4 custom-scrollbar">{activeSeason?.episodes?.map((e, idx) => (
                                <div key={idx} className="relative group">
                                    <div onClick={() => setActiveEp(idx)} className="p-8 border border-slate-800 bg-slate-900/30 rounded-[2rem] hover:border-teal-500 cursor-pointer text-center h-fit transition-all shadow-lg">
            ...
            {viewPersona && (
                <div className="fixed inset-0 z-[600] bg-black/95 flex items-center justify-center p-12 backdrop-blur-3xl" onClick={() => setViewPersona(null)}>
                    <div className="bg-[#0d0f11] w-full max-w-6xl h-[85vh] border border-teal-900/40 rounded-3xl p-10 flex gap-10 overflow-hidden relative shadow-2xl" onClick={e => e.stopPropagation()}>
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar pr-4 space-y-8 uppercase text-[10px]"><div className="p-8 bg-black/40 border border-slate-800 rounded-2xl leading-relaxed select-text italic text-teal-500 font-bold uppercase">CORE TRAUMA: {viewPersona?.dna?.core_trauma}</div><div className="p-8 bg-black/40 border border-slate-800 rounded-2xl leading-relaxed italic">{viewPersona?.dna?.personality_blurb}</div><div className="grid grid-cols-2 gap-6"><div className="space-y-4 border-l border-teal-900/30 pl-6 text-[9px] uppercase"><div className="font-black text-teal-500 mb-2 italic text-[10px]">Cultural Likes</div>{viewPersona?.dna?.likes?.map((l, i) => <span key={i} className="mr-2 opacity-70">[{l}]</span>)}</div><div className="space-y-4 border-l border-red-900/30 pl-6 text-[9px] uppercase"><div className="font-black text-red-500 mb-2 italic text-[10px]">Aversions</div>{viewPersona?.dna?.dislikes?.map((d, i) => <span key={i} className="mr-2 opacity-70">[{d}]</span>)}</div></div><div className="space-y-3 pt-6 border-t border-slate-800"><h4 className="text-teal-500 font-black uppercase italic text-[10px] mb-4">25 Core Memories</h4>{viewPersona?.dna?.core_memories?.map((m, i) => <div key={i} className="p-3 bg-white/5 rounded-xl text-[10px] italic leading-relaxed opacity-70">[{i+1}] {m}</div>)}</div></div>
                    </div>
                </div>
            )}
        </div>
