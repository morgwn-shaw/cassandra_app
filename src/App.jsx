...
const App = () => {
    ...
    const sync = async () => {
        ...
        } catch (e) { console.error("RAW_SYNC_ERR:", e.message); }
    };
    useEffect(() => { sync(); }, []);
    ...
    const ping = async () => {
        setLoad(true); try {
            ...
            if (!r.ok) throw new Error(`HTTP ${r.status}: ${d.error || 'Server Internal'}`);
            setEng(d.model?.toUpperCase()); setToday(d.date);
            window.alert(`RAW SIGNAL SUCCESS: ${d.model}`);
        } catch (e) { window.alert(`RAW SIGNAL FAIL: ${e.message}`); } finally { setLoad(false); }
    };
    ...
    const run = async (path, body, method = 'POST') => {
        setLoad(true); try {
            ...
            if (!contentType || !contentType.includes("application/json")) {
                ...
                throw new Error(`SERVER SENT HTML (Timeout/Crash): ${text.substring(0, 100)}...`);
            }
            const d = await r.json(); 
            if (d.error) throw new Error(`AI_ERROR: ${d.error}`);
            await sync(); return d;
        } catch (e) { 
            window.alert(`CRITICAL SIGNAL ERROR: ${e.message}`); 
        ...
        <div className="h-screen w-screen font-mono flex bg-[#0a0c0e] text-slate-400 overflow-hidden select-none text-[12px]">
            <aside className="w-[300px] border-r border-slate-800 bg-black/60 p-8 flex flex-col gap-6 shrink-0 shadow-2xl">
                <div className="border-b border-slate-900 pb-4">
                    <div className="flex items-center gap-3 text-teal-500 font-black text-sm uppercase tracking-widest"><Cpu size={16}/> Apex_v104.0</div>
                ...
                <div className="space-y-2">
                    <button onClick={ping} className="w-full p-4 border border-teal-900/30 text-teal-500 text-[10px] font-black uppercase hover:bg-teal-500 hover:text-black rounded transition-all shadow-lg"><Wifi size={14}/> Raw_Handshake</button>
                    <button onClick={() => setLoad(false)} className="w-full p-2 text-[9px] text-teal-900 font-black uppercase hover:text-teal-400 flex items-center justify-center gap-2"><RefreshCcw size={12}/> Clear Load State</button>
                </div>
            ...
            <main className="flex-1 flex flex-col p-12 overflow-y-auto relative bg-[#121416]">
                {load && <div className="absolute inset-0 bg-black/95 z-[100] flex flex-col items-center justify-center backdrop-blur-3xl"><Loader2 className="animate-spin text-teal-500 mb-2" size={32}/><p className="text-teal-500 text-[10px] font-black uppercase tracking-widest">Awaiting Raw Signal Response...</p></div>}
                <div className="flex gap-4 mb-10 border-b border-slate-800 pb-8"><button onClick={() => {setActive(null); setTab('season');}} className={`px-12 py-4 text-[11px] font-black ${tab === 'season' && !active ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>SEASONS</button><button onClick={() => {setActive(null); setTab('persona');}} className={`px-12 py-4 text-[11px] font-black ${tab === 'persona' && !active ? 'bg-teal-500 text-black shadow-lg shadow-teal-500/20' : 'bg-slate-800'}`}>DNA_VAULT</button></div>
                    ...
                    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-500">
                        ...
                        {tab === 'season' && (<div className="grid grid-cols-2 gap-10">
                            <div className="space-y-8">
                                <div className="bg-teal-950/10 p-10 border border-teal-900/30 rounded-3xl shadow-inner"><h4 className="text-teal-400 text-[10px] font-black uppercase mb-4 flex items-center gap-2 italic tracking-widest"><Globe size={14}/> 2026_Report</h4><p className="text-xl text-slate-300 leading-relaxed uppercase">{active?.summary || "ERR_NULL_SUMMARY"}</p></div>
